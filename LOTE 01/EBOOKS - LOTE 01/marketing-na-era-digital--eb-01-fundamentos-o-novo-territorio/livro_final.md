...
```mermaid
%% legenda: O Mix Digital: Produto, Preço, Praça e Promoção no Ambiente 
flowchart LR
  A[Consumidor conectado] --> B[Produto: customização, produtos di]
  B --> C[Preço: dinâmico, freemium e estrat]
  C --> D[Praça e promoção: multicanalidade,]
  D --> E[Decisão e conversão]
  E --> F[Medição e ajuste de rota]
  F --> B
```
...
```mermaid
%% legenda: A Economia da Atenção: Como Conquistar o Consumidor Distraíd
flowchart LR
  A[Consumidor conectado] --> B[A atenção como recurso escasso: cu]
  B --> C[Captura de atenção: gatilhos, jorn]
  C --> D[Consistência de marca como antídot]
  D --> E[Decisão e conversão]
  E --> F[Medição e ajuste de rota]
  F --> B
```
