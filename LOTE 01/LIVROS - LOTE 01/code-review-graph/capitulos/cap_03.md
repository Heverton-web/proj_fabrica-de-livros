...
```mermaid
%% legenda: Diagrama de propagação de blast radius em um sistema de e-commerce
%% O nó central (PaymentGateway) sofre alteração e o raio de impacto se propaga
%% pelas dependências diretas e indiretas, com cores indicando nível de risco.

flowchart TB
    subgraph "Alteração no PR"
        PG["PaymentGateway
        (ALTERADO)"]
    end

    subgraph "Blast Radius - Nível 1 (Impacto Direto)"
        OS["OrderService
        Risco: ALTO"]
    end

    subgraph "Blast Radius - Nível 2 (Impacto Indireto)"
        US["UserService
        Risco: MÉDIO"]
        RS["ReportService
        Risco: MÉDIO"]
    ...
    subgraph "Blast Radius - Nível 3 (Impacto Terciário)"
        RS2["ReportService
    ...
    OS -->|"import direto"| US
    OS -->|"import direto"| RS
    PG -.->|"dependência indireta via OrderService"| RS2

    style PG fill:#dc2626,color:#fff,stroke:#991b1b
    style OS fill:#ea580c,color:#fff,stroke:#c2410c
    style US fill:#d97706,color:#fff,stroke:#b45309
    style RS fill:#d97706,color:#fff,stroke:#b45309
    style RS2 fill:#65a30d,color:#fff,stroke:#4d7c0f
```

### 3.3.3 Fluxo do Workflow de Review com MCP

```mermaid
%% legenda: Fluxo completo de code review utilizando blast radius e ferramentas MCP
%% O desenvolvedor submete um PR, o sistema calcula o blast radius,
%% seleciona ferramentas MCP relevantes, e gera um relatório de impacto.

flowchart LR
    A["Desenvolvedor
    submete PR"] --> B["detect_changes
    compara branches"]
    B --> C["get_impact_radius
    calcula impacto"]
    C --> D["Seleção de
    ferramentas MCP"]
    D --> E["Análise de
    ...
    E --> F{"Blast Radius
    > limiar?"}
    F -->|"Sim"| G["Review humano
    obrigatório"]
    F -->|"Não"| H["Review por IA
    automatizado"]
    G --> I["Relatório de
    impacto gerado"]
    H --> I
...
```mermaid
%% legenda: Grafo de dependências com nós coloridos pelo peso de blast radius
%% Nós vermelhos têm alto impacto potencial; verdes, baixo impacto.

graph LR
    subgraph "Alto Blast Radius (>0.7)"
        A["ConfigLoader"]:::high
        B["HttpClient"]:::high
    end

    subgraph "Médio Blast Radius (0.3-0.7)"
        C["PaymentGateway"]:::medium
        D["DatabaseAdapter"]:::medium
    end

    subgraph "Baixo Blast Radius (<0.3)"
        E["EmailProvider"]:::low
        F["TemplateEngine"]:::low
        G["CacheLayer"]:::low
    end
    ...
    classDef high fill:#dc2626,color:#fff,stroke:#991b1b
    classDef medium fill:#d97706,color:#fff,stroke:#b45309
    classDef low fill:#16a34a,color:#fff,stroke:#15803d
```
