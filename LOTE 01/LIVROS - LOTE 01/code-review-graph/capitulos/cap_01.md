...
```mermaid
%% legenda: Fluxo de processamento do Code Review Graph — do diff ao contexto comprimido
flowchart LR
    A[Pull Request / Diff] --> B[Parser de AST]
    B --> C[Grafo de Dependencias]
    C --> D[Blast Radius BFS]
    D --> E[Filtragem por Tipo]
    E --> F[Deduplicacao]
    F --> G[Contexto Comprimido]
    G --> H[LLM para Review]
    H --> I[Comentario Estruturado]
...
```mermaid
%% legenda: Distribuicao do blast radius no Flask por profundidade de busca
graph TD
    A[app.py - Alterado] --> B[views.py]
    A --> C[wrappers.py]
    A --> D[ctx.py]
    B --> E[session.py]
    B --> F[blueprints.py]
    ...
    style F fill:#533483,stroke:#0f3460,color:#fff
    style G fill:#533483,stroke:#0f3460,color:#fff
    style H fill:#533483,stroke:#0f3460,color:#fff
