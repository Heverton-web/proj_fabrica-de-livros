...
#set page(
  ...
  header: context {
    if counter(page).get().first() > 1 {
      ...
      align(center, "Oh My Pi: Do Zero ao PhD em Coding Agents")
    }
  },
  footer: context {
    set text(size: 9pt)
...
#page(header: none, footer: none, numbering: none)[
  ...
  #align(center)[
    ...
    #v(3.5cm)
    #text(size: 20pt, weight: "bold")[Oh My Pi: Do Zero ao PhD em Coding Agents]
      ]
  #v(4cm)
    ...
    #block(
      ...
      #grid(
        ...
        [
          ...
          #pad(left: 0.8cm)[
            Oh My Pi: Do Zero ao PhD em Coding Agents \/ Heverton Eduardo Peres. --
            SÃ£o Paulo : FÃ¡brica AgÃªntica de Livros,
            2026.
          ]
          #pad(left: 0.8cm)[190 p. ; 21 cm.]
                    #v(0.15cm)
          #pad(left: 0.8cm)[ISBN 978-65-00000-25-2]
                    #v(0.15cm)
          #pad(left: 0.8cm)[1. Agente. 2. Coding. 3. Agents. 4. Sub-agentes. I. TÃ­tulo.]
          #v(0.3cm)
...
#heading(level: 1, outlined: false, numbering: none)[Capa]
<capa>
#strong[OH MY PI: DO ZERO AO PHD EM CODING AGENTS]
...
#heading(level: 1, outlined: false, numbering: none)[Folha de Rosto]
<folha-de-rosto>
#strong[Heverton Eduardo Peres]
...
#heading(level: 1, outlined: false, numbering: none)[Dedicatória]
<dedicatória>
#emph[Dedicado a todos os desenvolvedores curiosos que ousam transformar a forma como programam --- e a quem acredita que a colaboração humano-agente é o futuro da engenharia de software.]

#heading(level: 1, outlined: false, numbering: none)[Sumário]
<sumário>
#strong[Parte I --- Fundamentos: Conhecendo o Agente]

+ O que é um Coding Agent

+ Instalação e Configuração

#strong[Parte II --- Modo Interativo: O Agente ao Seu Lado]

#block[
#set enum(numbering: "1.", start: 3)
+ Primeiras Interações: Prompting Eficaz

+ Ferramentas do Agente: Read, Edit, Bash
]

#strong[Parte III --- Avançado: Sub-agentes e Automação]

#block[
#set enum(numbering: "1.", start: 5)
+ Sub-agentes: Paralelismo e Tarefas

+ Memória e Sessões
]

#strong[Parte IV --- Plugins e Extensões]

#block[
#set enum(numbering: "1.", start: 7)
+ Plugins: Expandindo o Agente

+ Skills: Conhecimento Especializado
]

#strong[Parte V --- Produção: Pipelines e DevOps]

#block[
#set enum(numbering: "1.", start: 9)
+ Automação de Pipelines

+ O Futuro dos Coding Agents
]
...
#heading(level: 1, outlined: false, numbering: none)[Prefácio]
<prefácio>
...
#heading(level: 1, outlined: false, numbering: none)[Parte I --- Fundamentos: Conhecendo o Agente]
<parte-i-fundamentos-conhecendo-o-agente>
#heading(level: 1, outlined: false, numbering: none)[Parte I --- Fundamentos: Conhecendo o Agente]
<parte-i-fundamentos-conhecendo-o-agente-1>
...
#figure(image("imagens/diagramas/dia_livro_01_7501bc2a7a.png", alt: "Ciclo de vida de uma tarefa em um coding agent"),
  caption: [
    Ciclo de vida de uma tarefa em um coding agent
  ]
)
...
#strong[Geração 1 --- Autocomplete (2021-2022).] Ferramentas como o GitHub Copilot original sugeream o próximo token com base no contexto imediato. O desenvolvedor ainda escrevia a maior parte do código e recebia sugestões pontuais. A IA era um assistente passivo \[7\].

#strong[Geração 2 --- Chat (2022-2023).] Chatbots como o ChatGPT permitiam fazer perguntas sobre código, receber explicações e gerar trechos isolados. Mas o chatbot não tinha acesso ao projeto do desenvolvedor --- era preciso copiar e colar código manualmente \[8\].

#strong[Geração 3 --- IDE Integrada (2023-2024).] Ferramentas como o Cursor integraram o LLM à IDE, permitindo que o agente lesse o arquivo corrente e gerasse código contextualizado. A interação ainda era majoritariamente reativa \[9\].

#strong[Geração 4 --- Agente Autônomo (2024-presente).] Coding agents como o Oh My Pi e o Claude Code operam de forma proativa e autônoma. Eles leem o projeto inteiro, executam comandos, rodam testes e encadeiam ações sem intervenção manual. O desenvolvedor passa de programador a supervisor \[10\].
...
#figure(
  align(center)[#table(
    ...
    table.header([Característica], [Oh My Pi (OMP)], [Claude Code], [Cursor], [GitHub Copilot], [Aider],),
    table.hline(),
...
#strong[Fonte:] Tabela compilada a partir da documentação oficial de cada ferramenta, consultada em julho de 2025 \[24\]\[25\]\[26\]\[27\]\[28\].
...
#strong[Cenário com chatbot convencional:]
...
#strong[A cena do erro --- o que acontece quando você não confia no agente o suficiente:]
...
#strong[O diagnóstico --- por que isso aconteceu:]
...
#strong[A prática correta --- como usar o agente da forma que ele foi projetado:]
...
=== Armadilhas comuns
<armadilhas-comuns>
A experiência do cenário acima revela padrões que se repetem em equipes que adotam coding agents. A primeira armadilha é #strong[subutilizar a autonomia do agente] --- tratar cada interação como uma pergunta isolada em vez de uma tarefa completa. A segunda é #strong[não delegar a validação] --- roda os testes manualmente quando o agente poderia fazer isso. A terceira é #strong[fornecer contexto insuficiente] --- pedir "corrija o bug" sem especificar onde ele ocorre ou como reproduzi-lo \[38\].
...
O Oh My Pi verifica as variáveis de ambiente em uma ordem específica: primeiro as variáveis dedicadas (ANTHROPIC\_API\_KEY, OPENAI\_API\_KEY), depois as variáveis genéricas (AI\_API\_KEY). Essa hierarquia permite configurar uma chave padrão e chaves específicas por provedor \[18\].
...
#figure(image("imagens/diagramas/dia_livro_02_f52e36d809.png", alt: "Fluxo de autenticação entre Oh My Pi e provedor de modelo"),
  caption: [
    Fluxo de autenticação entre Oh My Pi e provedor de modelo
  ]
)
...
#strong[A cena do erro --- o que acontece quando você ignora a ordem das etapas:]
...
#strong[O diagnóstico --- por que isso deu errado:]
...
#strong[A prática correta --- o setup em 7 passos:]
...
#heading(level: 1, outlined: false, numbering: none)[Parte II --- Modo Interativo: O Agente ao Seu Lado]
...
#strong[Contexto] é o "estado do mundo" que o agente precisa conhecer antes de executar. Em vez de digitar `corrige o bug`, o contexto diz: "Estou trabalhando no projeto `meu-app`, que é uma API REST em Python com FastAPI. O endpoint `/usuarios` retorna erro 500 ao receber e-mail com caractere especial." Sem contexto, o agente adivinha. Com contexto, o agente localiza o arquivo correto, entende o framework e sabe exatamente onde procurar o bug \[1\]\[5\].

#strong[Instrução] é o que você quer que o agente faça. Deve ser específica e acionável. "Analise o endpoint `/usuarios` e corrija o tratamento de e-mail para aceitar caracteres especiais sem retornar erro 500" é uma instrução clara. "Arruma isso aqui" é inútil. A instrução pode ser uma única ação ("adicione uma validação de e-mail") ou uma sequência ("leia o arquivo, identifique o bug, corra e rode os testes") \[2\]\[6\].

#strong[Restrições] são os limites dentro dos quais o agente deve operar. "Não modifique o schema do banco" ou "mantenha compatibilidade com Python 3.9" ou "use apenas a biblioteca `pydantic` para validação" são restrições que evitam que o agente tome decisões indesejadas. Restrições são especialmente importantes quando o projeto tem dependências legadas, padrões de código específicos ou requisitos de performance que o agente não poderia inferir sozinho \[5\]\[7\].

#strong[Formato] define como o agente deve entregar o resultado. "Retorne apenas o diff" ou "explique a mudança antes de aplicá-la" ou "gera um relatório em Markdown com os achados" --- o formato controla se você vai receber código puro, uma explicação detalhada ou um documento estruturado. No modo impressão (`-p`), o formato é particularmente importante porque a saída aparece no terminal e precisa ser consumível imediatamente \[8\].
...
#figure(image("imagens/diagramas/dia_livro_03_5731a62f99.png", alt: "Fluxo de execução de um prompt no Oh My Pi — das quatro camadas à ação concreta"),
  caption: [
    Fluxo de execução de um prompt no Oh My Pi --- das quatro camadas à ação concreta
  ]
)
...
#strong[Por que funciona:] em vez de um prompt gigante que tenta fazer tudo de uma vez, a decomposição permite que você verifique cada etapa antes de avançar. O agente executa a etapa 1, você confere a lista de variáveis, e só então avança para a etapa 2. Essa abordagem iterativa é o padrão recomendado para tarefas que envolvem múltiplos arquivos e múltiplas decisões \[7\]\[23\].

=== Dicas avançadas de prompting
<dicas-avançadas-de-prompting>
#strong[Use linguagem imperativa, não descritiva.] "Gere um endpoint REST para CRUD de usuários" é melhor que "eu gostaria de ter um endpoint REST para CRUD de usuários". O agente interpreta comandos diretos com mais precisão do que pedidos indiretos. Linguagem imperativa elimina ambiguidade: o agente sabe que precisa agir, não que precisa considerar uma possibilidade \[4\]\[6\].

#strong[Especifique o framework e a versão.] "Use FastAPI 0.110+" é melhor que "use um framework web". O agente pode escolher o framework errado ou usar uma versão desatualizada se você não especificar. Versões importam porque APIs mudam entre versões --- o que funciona em FastAPI 0.95 pode não funcionar em 0.110 \[2\]\[5\].

#strong[Valide o resultado.] Após receber o código gerado, peça ao agente que execute testes ou verifique a compilação. "Agora rode `pytest` e confirme que todos os testes passam" fecha o ciclo de geração-verificação. Sem validação, o agente pode reportar sucesso enquanto introduz bugs silenciosos \[19\]\[21\].

#strong[Referencie padrões existentes.] "Gere a função seguindo o padrão das funções já existentes em `src/utils/`" alinha o agente ao estilo do projeto, evitando código que funcione mas que quebre a consistência do codebase. Projetos grandes dependem de padrões para manter legibilidade --- e o agente deve respeitá-los \[6\]\[14\].

#strong[Use exemplos quando o padrão for ambíguo.] Se o agente precisa gerar código em um formato específico que não é padrão da linguagem, mostre um trecho existente como referência. Few-shot prompting é a técnica mais subutilizada por iniciantes --- e uma das mais eficazes para alinhar o agente ao estilo do seu projeto \[4\]\[6\].

#strong[Combine prompt com ação imediata.] Em vez de "analise o código e me diga o que está errado", use "analise o código, identifique o bug, aplique a correção e rode os testes". O agente pode executar múltiplas ações em sequência --- e prompts que combinam análise com ação produzem resultados mais rápidos do que prompts que pedem apenas análise \[7\]\[23\].

#strong[Use marcadores de seção para prompts longos.] Quando o prompt tem múltiplas partes, use marcadores visuais: "CONTEXTO: … INSTRUÇÃO: … RESTRIÇÕES: … FORMATO: …". Essa estrutura visual ajuda o agente a processar cada camada separadamente, mesmo em prompts com várias páginas \[5\]\[17\].

#strong[Evite ambiguidade temporal.] "Refatore o código" pode significar "refatore tudo agora" ou "planeje uma refatoração para depois". Seja explícito: "refatore o módulo de autenticação AGORA, aplicando as mudanças diretamente nos arquivos" \[1\]\[4\].
...
<erros-comuns-e-como-evitá-los>
#strong[Erro 1: Não definir escopo.] "Otimize o código" é perigoso porque o agente pode otimizar qualquer coisa --- desde um arquivo até o projeto inteiro. Sempre defina o diretório, os arquivos ou as funções alvo \[1\]\[24\].

#strong[Erro 2: Não informar o framework.] "Crie uma API REST" pode gerar código em Flask, Django, FastAPI, Express ou qualquer outro framework. Especificar a tecnologia evita retrabalho \[2\]\[5\].

#strong[Erro 3: Pedir "melhoria" sem definir métrica.] "Melhore a performance" é subjetivo. "Reduza o tempo de resposta do endpoint `/api/users` de 800ms para menos de 200ms" é mensurável e verificável \[4\]\[7\].

#strong[Erro 4: Não usar \@ para arquivos.] Colar o conteúdo de um arquivo no prompt é trabalhoso e sujeito a erros de formatação. Sempre use `@caminho/para/arquivo` quando o agente precisa ler um arquivo existente \[12\]\[14\].

#strong[Erro 5: Ignorar restrições negativas.] Sem dizer ao agente o que não fazer, ele pode "ajudar" demais --- removendo código que parece redundante, renomeando funções que outros módulos dependem ou alterando comportamentos que estão corretos. Restrições negativas são seu seguro contra mudanças indesejadas \[5\]\[7\].

#strong[Erro 6: Não validar o resultado.] Mesmo o melhor prompt pode gerar código com bugs sutis. Sempre peça ao agente que rode testes, linting ou verificação de tipo após gerar código. A verificação é o que fecha o ciclo de qualidade \[19\]\[21\].
...
#strong[Abordagem errada (um prompt gigante):]
...
#strong[Abordagem correta (série de prompts estruturados):]
...
Essa seleção não é aleatória. O agente segue uma lógica de decisão que pode ser resumida em três perguntas: preciso #strong[ler] algo ou #strong[escrever] algo? Se preciso ler, é um #strong[arquivo] específico ou uma #strong[busca] em vários arquivos? Se preciso escrever, é uma #strong[edição cirúrgica] ou uma #strong[criação do zero]? Cada resposta aponta para uma ferramenta diferente, e essa lógica de decisão é o que torna o agente eficiente --- ele não lê o projeto inteiro quando precisa ver uma função; ele não reescreve o arquivo inteiro quando precisa mudar uma linha \[1\]\[3\].
...
#figure(image("imagens/diagramas/dia_livro_04_70ba7a784a.png", alt: "Fluxo de decisao do Oh My Pi para selecao de ferramenta"),
  caption: [
    Fluxo de decisao do Oh My Pi para selecao de ferramenta
  ]
)
...
==== edit: substituição cirúrgica com old\_string e new\_string
...
#strong[Etapa 1 --- Leitura e compreensão:]
...
#strong[Etapa 2 --- Busca da lógica de conexão:]
...
#strong[Etapa 3 --- Leitura do trecho relevante:]
...
#strong[Etapa 4 --- Criação do novo módulo:]
...
#strong[Etapa 5 --- Edição do arquivo original:]
...
#strong[Etapa 6 --- Busca por referências restantes:]
...
#strong[Etapa 7 --- Verificação final:]
...
#strong[O erro clássico: usar bash cat em vez de read]
...
#strong[O padrão correto: read + grep + edit]
...
=== Erros comuns e como evitá-los
<erros-comuns-e-como-evitá-los-1>
#strong[Erro 1: Usar bash para tudo.] O `bash` é a ferramenta mais versátil, mas não a mais segura. Usar `bash grep` em vez da ferramenta `grep` dedicada gera saída bruta que o agente precisa processar manualmente. Usar `bash cat` em vez de `read` desperdiça tokens. Use `bash` apenas quando nenhuma ferramenta dedicada existe \[1\]\[14\].

#strong[Erro 2: Usar write para modificar arquivos existentes.] O `write` sobrescreve o arquivo inteiro. Se houver um erro de formatação no conteúdo escrito --- uma vírgula faltando, um parênteses desalinhado --- o arquivo inteiro pode ser corrompido. Use `edit` para modificações pontuais \[12\]\[13\].

#strong[Erro 3: Não usar offset/limit no read.] Ler um arquivo de 10.000 linhas inteiramente é desperdício de contexto. Sempre que souber quais linhas precisa, use `offset` e `limit` para carregar apenas o trecho relevante \[4\]\[5\].

#strong[Erro 4: Não verificar o resultado.] Após modificar um arquivo, sempre rode verificação: compilação, testes, linting. O agente pode aplicar uma edição corretamente mas introduzir um bug sutil --- e a verificação é o que pega esses bugs antes que causem problemas \[19\]\[21\].

#strong[Erro 5: Não usar workdir no bash.] Executar comandos sem definir o diretório de trabalho pode causar efeitos colaterais em diretórios errados. Sempre especifique `workdir` quando o comando depende do contexto do projeto \[15\]\[16\].

#strong[Erro 6: Não ler antes de escrever.] Antes de usar `write` em um arquivo existente, sempre leia o conteúdo atual. Essa disciplina previne perda acidental de código e garante que o novo conteúdo mantém a estrutura esperada \[4\]\[12\].
...
#strong[Abordagem errada:] usar o `bash` para tudo.
...
#strong[Abordagem correta:] cadeia de ferramentas dedicadas.
...
#heading(level: 1, outlined: false, numbering: none)[Parte III --- Avançado: Sub-agentes e Automação]
...
== 3. Ilustra
<ilustra-5>
Pense na orquestração do OMP como a gestão de uma oficina de manutenção com vários mecânicos. O agente principal é o mestre de obras: ele recebe o pedido do cliente ("concerte a bomba d'água e verifique o sistema elétrico"), decompõe em tarefas (T1: diagnosticar bomba, T2: inspecionar fiação), asigna cada tarefa a um mecânico especializado e acompanha o progresso. A tool `task` é a prancheta do mestre de obras --- onde ele anota as tarefas, seus status e quem está responsável. A tool `actor` é o interfone pelo qual ele despacha os mecânicos: `run` é ligar e falar até o mecânico terminar; `spawn` é mandar um recado e deixar o mecânico trabalhar enquanto ele atende outro pedido. Os mecânicos `explore` são os estagiários --- só olham, não mexem; os mecânicos `general` são os especialistas --- mexem, consertam e reportam. O binding entre tarefa e mecânico é o crachá: cada mecânico com seu crachá (task\_id) sabe exatamente qual reparo está fazendo, e o mestre de obras sabe onde buscar o resultado.

#figure(image("imagens/diagramas/dia_livro_05_8d21e2bf9a.png", alt: "A oficina de orquestracao: mestre de obras despacha mecanicos por tarefa"),
  caption: [
    A oficina de orquestracao: mestre de obras despacha mecanicos por tarefa
  ]
)

#figure(image("imagens/ilustracoes/ilust_05_1.png", alt: "Ilustração Capítulo 5"),
  caption: [
    Ilustração Capítulo 5
  ]
)

Repare no diagrama como o mestre de obras (agente principal) mantém dois fluxos simultâneos: a prancheta do plano (task) e a coordenação dos mecânicos (actor). O binding via task\_id conecta o resultado de cada mecânico de volta à tarefa certa na prancheta. A beleza do paralelismo aparece quando o mestre despacha T1 e T2 ao mesmo tempo --- dois mecânicos trabalham em enquanto ele continua orquestrando. A mesma estrutura escala: uma refatoração grande pode ter T1, T2, T3 e T4 despachados simultaneamente, cada um com seu sub-agente, e o mestre apenas acompanha os status até todos marcarem `done`.
...
#figure(image("imagens/diagramas/dia_livro_06_c879649a81.png", alt: "Maquina de estados do ciclo de vida de uma tarefa"),
  caption: [
    Maquina de estados do ciclo de vida de uma tarefa
  ]
)
...
Retorna todas as tarefas ativas (open, in\_progress, blocked) --- o dashboard do plano. Para ver tarefas concluídas ou arquivadas, passe `include_terminal: true`. A consulta por ID (`get`) retorna o estado detalhado de uma tarefa específica --- útil quando o agente precisa recordar o que foi planejado para T3 antes de despachar um sub-agente \[1\].
...
#strong[run --- execução bloqueante:] O sub-agente é criado, executa a tarefa e devolve o resultado inline. O agente principal bloqueia até a conclusão. É a escolha quando o resultado é necessário para prosseguir:
...
#strong[status --- consulta não bloqueante:] Verifica o estado de um actor sem bloquear. Retorna `{ status: "pending"|"running"|"idle"|"unknown", actor_id, turnCount }`. É a escolha para polling periódico quando o agente principal quer saber se um background job terminou sem bloquear a execução \[2\].

#strong[cancel --- interrupção graciosa:] Interrompe um actor em execução. É idempotente --- cancelar um actor já cancelado não gera erro. O cancelamento é gracioso: o sub-agente recebe o sinal e pode salvar estado antes de encerrar \[2\].

#strong[send --- mensagem ao inbox:] Entrega uma mensagem ao inbox de um actor. O receiver vê a mensagem envolta em `<inbox>` no próximo turno. É útil para enviar atualizações a um sub-agente que está aguardando input, ou para coordenar entre sub-agentes \[2\]\[3\]:
...
#figure(
  align(center)[#table(
    ...
    table.header([Cenário], [Tipo], [Justificativa],),
    table.hline(),
...
=== Sub-agente vinculado a task: o fluxo completo
<sub-agente-vinculado-a-task-o-fluxo-completo-1>
O fluxo completo de uma tarefa com binding segue cinco passos. Primeiro, criar a tarefa com `task`. Segundo, despachar o sub-agente com `task_id`. Terceiro, o sub-agente executa e escreve progresso em `tasks/<TID>/progress.md`. Quarto, o checkpoint-writer integra o progresso no checkpoint. Quinto, o agente principal verifica o resultado e marca a tarefa como `done` \[1\]\[2\]:
...
#figure(image("imagens/diagramas/dia_livro_07_883317a2da.png", alt: "O sistema de memoria do OMP como escritorio com arquivos e turnos"),
  caption: [
    O sistema de memoria do OMP como escritorio com arquivos e turnos
  ]
)

#figure(image("imagens/ilustracoes/ilust_06_1.png", alt: "Ilustração Capítulo 6"),
  caption: [
    Ilustração Capítulo 6
  ]
)
...
#strong[Nova sessão (padrão):] Ao iniciar o OMP sem flags, uma nova sessão é criada. O contexto começa limpo --- exceto pela injeção automática de `MEMORY.md` e do checkpoint mais recente (se existir). O agente começa sabendo o que lembra do projeto, mas não tem histórico de conversa anterior.

#strong[--continue:] Retoma a sessão mais recente do diretório atual. O histórico de conversa é restaurado, o checkpoint é injetado e o agente continua de onde parou. É a escolha para trabalho contínuo em um projeto --- você fecha o terminal, volta no dia seguinte, `mimo --continue` e está de volta.

#strong[--resume:] Permite escolher qual sessão retomar (o OMP lista as sessões disponíveis com timestamps e títulos). É a escolha quando há múltiplas sessões ativas e você precisa voltar a uma específica.

#strong[--session-dir:] Define um diretório personalizado para armazenar dados da sessão. Útil para isolar sessões de teste ou para projetos com múltiplos worktrees.

#strong[--no-session:] Desativa a persistência de sessão. O OMP opera sem salvar estado --- útil para comandos rápidos ou scripts que não precisam de memória entre chamadas.
...
#strong[Compaction:] Quando o contexto se aproxima do limite (tipicamente 80-90% da capacidade do modelo), o OMP comprime mensagens antigas em resumos. A compaction é inteligente: preserva decisões, resultados de tools e erros; descarta narração, explicações e mensagens de baixa prioridade. O resultado é um contexto menor que mantém o essencial. O agente não é notificado quando a compaction acontece --- ele trata o contexto visível como a fonte de verdade. Isso significa que conversas antigas podem estar resumidas, e o agente deve reconstruir contexto a partir do que está disponível \[2\]\[5\].

#strong[Overflow:] Quando a compaction não é suficiente, mensagens que excedem o limite são descartadas. É o fallback --- menos sofisticado que a compaction, mas garantido para manter o contexto dentro do limite. Mensagens descartadas são as mais antigas, e o agente perde acesso a detalhes dessas interações \[2\].

#strong[Pruning:] Results de tools grandes (logs de build, outputs de grep, dumps de JSON) são truncados ou removidos para liberar espaço. O pruning é seletivo: mantém as primeiras 3 e últimas 4 linhas de outputs grandes (regra headroom), preservando o início e o fim que contêm as informações mais relevantes \[5\].
...
+ Lê o estado atual da sessão (tarefas, tools recentes, mensagens)
+ Sintetiza um snapshot compacto
+ Escreve em `checkpoint.md`
+ Integra descobertas de sub-agentes vinculados a tarefas (de `tasks/<TID>/progress.md`)
...
#strong[Queries curtas e específicas:] "T5.3 closure" funciona. "config params database connection" gera ruído. A regra é usar 1-3 termos distintos --- o raro pesa mais que o comum.

#strong[Punctuation stripping:] Pontuação (`.`, `-`, `/`, `:`) é removida na tokenização. `T5.3` casa com `T5.3`, `T5_3` ou `T5 3`. Uma URL como `postgres://host:5433` é indexada como tokens `postgres`, `host`, `5433` --- busque um dos três, não a URL inteira.

#strong[Hits autoritativos:] Se a busca retorna um resultado, confie nele --- mesmo que outra query tenha retornado nada. Não conclua "nunca registrei isso" porque uma formulação falhou.

#strong[Partial hits:] Um hit pode dar o gisto mas ter paraphraseado um valor exato. Para recall literal, consulte o `history` --- a mensagem original tem o valor verbatim.

#strong[Escalation quando 0 resultados:] (1) Retry com menos termos. (2) Grep direto no diretório de memória. (3) Ampliar scope: session → project → global → history.
...
Essa integração é o que torna o sistema de tarefas persistente não apenas em estado (open/in\_progress/done) mas em conteúdo (o que foi feito, o que foi descoberto). O plano não é apenas uma lista de checkbox --- é um registro vivo do trabalho \[1\].
...
#heading(level: 1, outlined: false, numbering: none)[Parte IV --- Plugins e Extensões]
...
O sistema de plugins do OMP opera com três conceitos centrais. O primeiro é o #strong[plugin dir] --- o diretório onde os plugins vivem, tipicamente em `.opencode/plugins/` dentro do projeto ou em `~/.opencode/plugins/` para plugins globais. Cada plugin é um diretório com um manifesto (metadados, dependências, ponto de entrada) e código executável. O segundo conceito é o #strong[omp install] --- o comando que baixa, instala e habilita um plugin a partir de um repositório ou caminho local. O terceiro são os #strong[extensions] --- extensões de maior granularidade que podem ser ativadas por sessão com `-e` ou `--extension`, ou desativadas completamente com `--no-extensions` \[1\]\[2\].
...
Hooks são o segundo mecanismo de extensão, e são mais granulares que plugins. Enquanto um plugin adiciona uma capacidade inteira (um novo comando slash ou uma nova ferramenta), um hook intercepta um momento específico do ciclo de vida do agente e injeta comportamento. O OMP suporta quatro categorias de hooks. #strong[Pre-commit hooks] rodam antes do agente gravar um arquivo --- são o portão de entrada que pode validar, formatar ou bloquear. #strong[Post-commit hooks] rodam após a gravação --- são o verificador que pode rodar testes, atualizar caches ou notificar. #strong[Tool call hooks] interceptam cada chamada de ferramenta antes de sua execução --- são o filtro que pode bloquear comandos perigosos, redirecionar saídas ou logar ações. E #strong[session hooks] disparam no início e no fim da sessão --- são o ritual de setup e teardown \[1\]\[3\].
...
#figure(image("imagens/diagramas/dia_livro_08_8dc2269c8b.png", alt: "A oficina extensível: plugins, hooks e extensions no ciclo de vida do agente"),
  caption: [
    A oficina extensível: plugins, hooks e extensions no ciclo de vida do agente
  ]
)

#figure(image("imagens/ilustracoes/ilust_07_1.png", alt: "Ilustração Capítulo 7"),
  caption: [
    Ilustração Capítulo 7
  ]
)
...
#strong[Pre-commit hooks] rodam antes de qualquer gravação de arquivo. O caso de uso clássico é impedir que segredos entrem no repositório:
...
#strong[Post-commit hooks] rodam após uma ação do agente (gravação de arquivo, execução de comando). O caso de uso clássico é rodar testes automaticamente para garantir que a edição não quebrou nada:
...
<extensions--eextension-e-no-extensions>
...
+ #strong[Session start] --- hooks de inicialização rodam quando a sessão do agente começa
+ #strong[Tool call] --- antes de cada chamada de ferramenta, o tool-call hook executa
+ #strong[Pre-commit] --- antes de gravar um arquivo, o pre-commit hook executa
+ #strong[File write] --- o arquivo é gravado no disco
+ #strong[Post-commit] --- após a gravação, o post-commit hook executa
+ #strong[Session end] --- hooks de finalização rodam quando a sessão termina
...
== References
<references>
- #link("https://mosquitto.org/documentation/")[Mosquitto docs]
- #link("https://docs.oasis-open.org/mqtt/mqtt/v5.0/")[MQTT 5.0 spec]
...
#figure(
  align(center)[#table(
    ...
    align: (auto,auto,auto,),
    table.header([Skill], [Função], [Trigger típico],),
    table.hline(),
...
<compose-skills-composeplan-e-composeexecute>
...
=== Skills de domínio: IoT, segurança e deploy
<skills-de-domínio-iot-segurança-e-deploy-1>
...
<ciclo-de-vida-das-skills-criação-validação-evolução>
Skills seguem um ciclo de vida contínuo. A #strong[criação] acontece quando o agente (ou o usuário) identifica um workflow que vale preservar --- o `skill-creator` extrai o procedimento e formata como SKILL.md. A #strong[validação] acontece quando a skill é usada pela primeira vez em contexto real --- se o agente segue os passos e o resultado é correto, a skill está validada; se falha, ela é revisada com `writing-skills`. A #strong[evolução] acontece quando o domínio muda --- o Mosquitto lança uma nova versão, o OMP adiciona uma nova ferramenta, o projeto muda de stack --- e a skill é atualizada para refletir o novo estado \[1\]\[3\]\[6\]:
...
O Oh My Pi implementa essa escala com três mecanismos nativos. O #strong[cron tool] agenda tarefas que disparam automaticamente em horários ou intervalos regulares -- o relógio do agente. Os #strong[hooks] interceptam cada chamada de ferramenta antes ou depois dela acontecer -- o sistema nervoso do agente, que reage a estímulos sem intervenção manual. E os #strong[workflows] orquestram múltiplos sub-agentes com controle de fluxo determinístico -- a coluna vertebral da automação complexa \[3\]\[4\].

=== O cron tool: o relógio do agente
<o-cron-tool-o-relógio-do-agente>
O cron tool do Oh My Pi é uma abstração sobre o agendador cron do sistema operacional, mas com uma diferença fundamental: em vez de agendar comandos shell, ele agenda #strong[prompts] -- instruções em linguagem natural que o agente interpreta e executa quando o relógio dispara. Dois modos de operação coexistem. O modo #strong[schedule] usa uma expressão cron de 5 campos (minuto, hora, dia do mês, mês, dia da semana) e dispara o prompt na cadência fixa -- "todo dia às 9h", "a cada 5 minutos". O modo #strong[loop] é um timer de delay único com keepalive: o agente dispara, executa e, se o usuário quiser que repita, chama o loop de novo no turno seguinte, renovando o timer. A diferença prática: schedule é para cadências conhecidas e estáticas; loop é para cadências que dependem do que o agente observou \[3\]\[5\].

A distinção entre jobs #strong[durable] e #strong[session-only] determina a longevidade do agendamento. Um job session-only morre quando a sessão do REPL termina -- útil para monitores temporários e babysitting de PRs. Um job durable persiste entre sessões, gravado em disco, e sobrevive a restarts -- ideal para check-ins diários, relatórios de status e vigília de deploys. A escolha entre durable e session-only é a mesma decisão que o desenvolvedor toma entre um cronjob permanente e um script rodata uma vez: a permanência tem custo de manutenção, mas elimina a necessidade de recriar a cada sessão \[3\]\[5\].

=== Os hooks: o sistema nervoso do agente
<os-hooks-o-sistema-nervoso-do-agente>
Hooks são pontos de interceptação no ciclo de vida do agente -- momentos em que uma ação pode ser observada, modificada ou bloqueada antes de acontecer. O Oh My Pi implementa hooks em dois níveis. No nível de #strong[ferramenta], um hook `pre_tool_call` roda antes de qualquer chamada de ferramenta (bash, edit, write) e pode bloquear a ação com base em regras -- por exemplo, impedir que o agente rode `git push` sem testes passando. Um hook `post_tool_call` roda depois da execução e pode registrar, reagir ou até reverter -- por exemplo, logar toda edição de arquivo para auditoria. No nível de #strong[sessão], hooks de lifecycle disparam no início e no fim da sessão, no primeiro turno e na compactação de contexto -- pontos ideais para restaurar estado, enviar relatórios ou preparar o ambiente \[2\]\[6\].
...
#figure(image("imagens/diagramas/dia_livro_10_7aaccb085a.png", alt: "A fábrica de automação: cron, hooks, workflows e sub-agentes orquestrando a produção"),
  caption: [
    A fábrica de automação: cron, hooks, workflows e sub-agentes orquestrando a produção
  ]
)

#figure(image("imagens/ilustracoes/ilust_09_1.png", alt: "Ilustração Capítulo 9"),
  caption: [
    Ilustração Capítulo 9
  ]
)
...
<integração-com-github-actions>
A integração com GitHub Actions permite que o Oh My Pi opere como membro do pipeline CI/CD. O padrão mais comum é o #strong[agente como desenvolvedor]: um workflow do GitHub Actions dispara o Oh My Pi com um prompt, o agente executa o trabalho e faz commit de volta. O trigger pode ser um issue atribuído, um PR que precisa de review ou um schedule que verifica o estado do repositório \[1\]\[8\]:
...
<multi-agent-orchestration-padrões-de-coordenação>
A orquestração multi-agente tem quatro padrões fundamentais, cada um resolvendo um tipo diferente de problema. O padrão #strong[fan-out/fan-in] despacha N sub-agentes para N tarefas independentes e consolida os resultados -- ideal para revisão de múltiplos arquivos, análise de múltiplos issues ou processamento de lote. O padrão #strong[pipeline] encadeia sub-agentes sequenciais, onde a saída de um é a entrada do outro -- ideal para análise → decisão → implementação. O padrão #strong[supervisor] mantém um agente coordenador que monitora e redireciona sub-agentes -- ideal para tarefas longas com pontos de verificação. E o padrão #strong[negotiation] usa dois ou mais sub-agentes que discutem um problema e convergem em uma solução -- ideal para decisões de arquitetura \[4\]\[9\]:
...
<métricas-de-sucesso-da-automação>
Uma pipeline automatizada se mede por três linhas. A primeira é #strong[tempo economizado] -- quantas horas por semana a automação substituiu de trabalho manual (se o cron de status substitui 30 minutos de check-in manual, o retorno é claro). A segunda é #strong[custo de tokens] -- quanto a automação gasta em comparação com o valor que gera (um workflow de revisão que custa \$2 de tokens mas evita um bug de \$200 em produção é um retorno de 100x). A terceira é #strong[taxa de falsos positivos] -- quantas vezes o hook bloqueou algo legítimo ou o workflow gerou irrelevâncias (a meta é zero; acima de 5% é sinal de que as regras precisam de ajuste) \[3\]\[5\].
...
A evolução de assistente a colaborador tem três dimensões. A primeira é #strong[escopo]: o assistente edita uma linha; o colaborador modifica um módulo inteiro, cria testes e abre um PR. A segunda é #strong[iniciativa]: o assistente espera o prompt; o colaborador identifica o problema e propõe uma solução. A terceira é #strong[responsabilidade]: o assistente não é culpado por erros; o colaborador é avaliado pela qualidade do trabalho. Essa transição é gradual e depende do contexto: em projetos pequenos e bem definidos, a autonomia é alta; em projetos grandes e ambíguos, a supervisão humana continua essential \[3\]\[4\].

=== Tendências: agentes autônomos, multi-modais e self-improving
<tendências-agentes-autônomos-multi-modais-e-self-improving>
Três tendências dominam o horizonte de 2025-2030. A primeira é a #strong[autonomia crescente]. Os agentes estão ganhando a capacidade de manter tarefas de longa duração -- horas ou dias de trabalho contínuo, com checkpoints e recuperação de falhas. O Oh My Pi já suporta jobs duráveis e workflows de longa duração; a próxima fronteira é o agente que mantém um projeto inteiro -- fazendo commits, revisando PRs, monitorando deploy -- sem intervenção humana por dias. A pesquisa em agentes autônomos avança rápido: o AutoGPT, o Devin e o OpenDevin demonstram a viabilidade técnica, mas a confiabilidade ainda é o gargalo -- o agente que trabalha sozinho precisa ser confiável o suficiente para que o humano durma tranquilo sabendo que o código está sendo modificado \[5\]\[6\].

A segunda tendência é a #strong[multi-modalidade]. Os coding agents atuais trabalham com texto -- prompts, código, documentação. Os agentes multi-modais trabalham com texto, imagem, áudio e vídeo ao mesmo tempo. No contexto de desenvolvimento, isso significa: o agente que lê um screenshot de uma UI e implementa o componente visual, o agente que escuta uma reunião de standup e gera tarefas no issue tracker, o agente que analisa um diagrama de arquitetura desenhado num quadro branco e produz o código correspondente. Os modelos de base já suportam entrada multi-modal (GPT-4o, Claude 3.5, Gemini 1.5); o que falta é a integração nos workflows de desenvolvimento \[1\]\[7\].

A terceira tendência é o #strong[self-improvement] -- agentes que melhoram a si mesmos. Duas abordagens coexistem. A primeira é a aprendizagem por feedback: o agente recebe feedback do humano (approvals, rejeções, correções) e ajusta seu comportamento futuro -- por exemplo, aprender que este projeto prefere testes unitários em vez de integração, ou que este time usa Conventional Commits. A segunda é a evolução de skills: o agente identifica padrões nas tarefas que executa e cria novas skills automaticamente -- o que a skill `self-learning` do Oh My Pi já implementa em forma rudimentar \[8\]\[9\].

=== Ética e segurança: data privacy, guardrails e human-in-the-loop
<ética-e-segurança-data-privacy-guardrails-e-human-in-the-loop>
A autonomia dos agentes levanta questões éticas e de segurança que não são abstratas -- são requisitos de engenharia. A primeira dimensão é #strong[privacidade de dados]. Um coding agent tem acesso ao código-fonte completo de um projeto -- que pode conter chaves de API, credenciais de banco de dados e lógica proprietária. O envio desse código para um modelo de linguagem em nuvem levanta a questão: quem mais pode ver o meu código? As respostas variam: modelos locais (Ollama, LM Studio) processam tudo na máquina do desenvolvedor; modelos em nuvem (Anthropic, OpenAI) processam nos servidores do provedor, com políticas de retenção e uso que variam entre empresas. O Oh My Pi suporta ambos os modos -- a escolha é uma decisão de risco que cada organização deve tomar \[10\]\[11\].

A segunda dimensão é #strong[guardrails] -- restrições programáticas que limitam o que o agente pode fazer. Guardrails são os hooks do Capítulo 9 elevados a política organizacional: "o agente não pode modificar arquivos de configuração de produção", "o agente não pode fazer push para main", "o agente não pode acessar dados de clientes". A implementação técnica é a mesma -- hooks pre\_tool\_call com decisões deny -- mas o contexto é organizacional: os guardrails definem os limites da autonomia, e esses limites devem ser claros, auditáveis e revisáveis \[10\]\[12\].

A terceira dimensão é o #strong[human-in-the-loop] -- a exigência de que decisões críticas passem por aprovação humana antes de serem executadas. O padrão é: o agente propõe, o humano aprova. A implementação é o hook `ask` que o Oh My Pi já suporta -- em vez de `deny` (bloqueio total) ou `allow` (execução automática), o hook pergunta ao usuário. A questão é onde desenhar a linha: o agente pode criar branches e commits sem aprovação? Pode abrir PRs? Pode fazer deploy? Cada organização define sua própria fronteira, e essa fronteira deve ser documentada e comunicada ao agente \[10\]\[12\].
...
A arquitetura MCP tem três componentes. O #strong[servidor MCP] é o processo que expõe ferramentas e recursos -- por exemplo, um servidor que conecta ao GitHub e expõe `list_issues`, `create_pr`, `get_file_content` como ferramentas. O #strong[cliente MCP] é o agente que consome essas ferramentas -- o Oh My Pi, por exemplo, pode ser um cliente MCP que usa servidores de GitHub, Slack, Jira e banco de dados. E o #strong[transporte] é o canal de comunicação entre cliente e servidor -- tipicamente stdio (processo local) ou HTTP/SSE (serviço remoto) \[13\]\[14\]:
...
Os novos papéis que emergem incluem o #strong[Prompt Engineer] (especialista em comunicação eficaz com agentes), o #strong[Agent Ops Engineer] (responsável pela operação e monitoramento de agentes em produção), o #strong[Guardrails Engineer] (responsável por definir e implementar as restrições de segurança dos agentes) e o #strong[AI Auditor] (responsável por avaliar a qualidade, viés e conformidade das saídas dos agentes). Cada papel é uma especialização do desenvolvedor, não uma substituição \[15\]\[16\].
...
A pesquisa em agentes de código é ativa e rápida. Três linhas de investigação dominam. A primeira é a #strong[resolução de bugs]: agentes que recebem uma issue, analisam o código-fonte, reproduzem o bug e propõem uma correção -- com taxas de sucesso que variam de 30% a 80% dependendo da complexidade \[3\]\[4\]. A segunda é a #strong[geração de código]: agentes que recebem uma especificação em linguagem natural e produzem implementação completa, incluindo testes e documentação -- com qualidade que já atinge o nível de PR aceitável em projetos simples \[5\]\[6\]. A terceira é a #strong[manutenção de código]: agentes que monitoram um repositório, detectam deprecations, atualizam dependências e mantêm o código atualizado -- uma tarefa que consome 20-30% do tempo de desenvolvimento em projetos maduros \[9\]\[19\].
...
As defesas seguem o modelo de defesa em profundidade. A primeira camada é a #strong[minimização de permissões]: o agente deve ter apenas as permissões necessárias para a tarefa -- não acesso total ao sistema. A segunda camada são os #strong[guardrails programáticos]: hooks que bloqueiam ações perigosas antes de acontecerem. A terceira camada é a #strong[auditoria]: logs de todas as ações do agente, revisáveis por humanos. A quarta camada é o #strong[sandboxing]: executar o agente num ambiente isolado (container, VM) onde o dano é contido. A quinta camada é a #strong[verificação humana]: decisões críticas passam por aprovação antes de execução \[10\]\[12\]\[20\].
...
#figure(image("imagens/diagramas/dia_livro_11_fa2e1d80bb.png", alt: "A evolução do coding agent: de assistente a sócio, com as camadas que sustentam cada nível"),
  caption: [
    A evolução do coding agent: de assistente a sócio, com as camadas que sustentam cada nível
  ]
)

#figure(image("imagens/ilustracoes/ilust_10_1.png", alt: "Ilustração Capítulo 10"),
  caption: [
    Ilustração Capítulo 10
  ]
)
...
<agentes-autônomos-arquitetura-e-limitações>
A arquitetura de um agente autônomo tem cinco componentes: o #strong[LLM] (cérebro que planeja e decide), as #strong[ferramentas] (mãos que executam -- read, edit, bash, MCP), a #strong[memória] (contexto que persiste entre turnos), o #strong[orçamento de tokens] (recurso finito que limita a autonomia) e os #strong[guardrails] (restrições que definem o que pode ser feito). A autonomia do agente é proporcional à qualidade desses cinco componentes -- um LLM fraco, ferramentas limitadas, memória pequena, orçamento apertado e guardrails ausentes resultam num agente que ou trava ou faz besteira \[3\]\[5\]:
...
As limitações práticas dos agentes autônomos são quatro. A primeira é a #strong[degradação de contexto]: em conversas longas, o contexto acumulado sobrecarrega o modelo, e a compactação (resumo de mensagens antigas) perde informação crítica. A segunda é a #strong[cascata de erros]: um erro no turno 5 pode ser corrigido no turno 6, mas se o agente não perceber o erro, ele compõe sobre ele nos turnos seguintes, e a correção fica cada vez mais difícil. A terceira é o #strong[custo acumulado]: um agente autônomo que roda por 50 turnos consome tokens significativos, e o custo pode superar o valor da tarefa. A quarta é a #strong[falta de julgamento sobre limites]: o agente pode tentar resolver um problema que está além da sua capacidade, gastando tokens sem avançar, quando a resposta certa seria pedir ajuda humana \[3\]\[5\]\[6\].
...
<self-improvement-aprendizagem-e-criação-de-skills>
O self-improvement em coding agents tem duas implementações concretas. A primeira é o #strong[feedback loop]: o agente registra suas ações e os resultados, e usa esse registro para ajustar comportamento futuro. No Oh My Pi, isso se materializa na memória de projeto (MEMORY.md) e na skill `self-learning` que captura "golden paths" -- sequências de ações que funcionaram e devem ser reutilizadas. A segunda é a #strong[criação autônoma de skills]: o agente identifica um padrão recorrente (uma sequência de 3+ passos que ele executa manualmente repetidamente) e extrai em uma skill reutilizável \[8\]\[9\]:
...
#figure(
  align(center)[#table(
    columns: (33.33%, 33.33%, 33.33%),
    align: (auto,auto,auto,),
    table.header([Servidor], [O que faz], [Uso típico],),
    table.hline(),
...
As defesas contra prompt injection são múltiplas e devem ser usadas em camadas. A primeira é a #strong[separação de confiança]: tratar código-fonte como dado, não como instrução -- o agente deve processar o código para entender sua lógica, mas nunca interpretar comentários ou strings como comandos. A segunda é a #strong[validação de saída]: verificar se a ação que o agente vai executar é consistente com o que o usuário pediu -- se o usuário pediu "adicionar uma função" e o agente vai "ler \~/.ssh/id\_rsa", algo está errado. A terceira é o #strong[sandboxing]: executar o agente num ambiente onde o dano é contido (container Docker, VM) -- mesmo que o prompt injection funcione, o dano fica no sandbox \[10\]\[12\]\[20\]:
...
<métricas-de-sucesso-na-era-dos-agentes>
Um time que usa coding agents se mede por quatro linhas. A primeira é #strong[velocidade de entrega] -- quantas features por sprint, comparado com o time sem agentes (a meta é 2-3x sem perda de qualidade). A segunda é #strong[qualidade do código] -- taxa de bugs em produção, cobertura de testes, violações de estilo (a meta é manter ou melhorar). A terceira é #strong[custo total] -- tokens de IA + tempo humano + infraestrutura (o agente deve reduzir o custo total, não apenas deslocá-lo de salário para tokens). A quarta é #strong[segurança] -- incidentes de vazamento, prompts injection detectados, ações bloqueadas por guardrails (a meta é zero incidentes, não zero bloqueios) \[3\]\[5\]\[15\].
...
\[1\] ABRAMS, Steven et al.~Security considerations for AI-assisted software development. #emph[IEEE Software], v. 41, n.~3, 2024. Disponível em: https:/\/ieeexplore.ieee.org/document/10457529. Acesso em: 15 jul. 2025.

\[2\] ACM. #emph[Computing Curricula --- knowledge skills in CS education.] Disponível em: https:/\/www.acm.org/education/curricula-recommendations. Acesso em: 4 ago. 2026.
...
\[28\] ANTHROPIC. #emph[Claude Code --- sub-agents and parallel tool use.] Disponivel em: https:/\/docs.anthropic.com/. Acesso em: 4 ago. 2026.

\[29\] ANTHROPIC. #emph[Claude Code --- memory and context management.] Disponivel em: https:/\/docs.anthropic.com/. Acesso em: 4 ago. 2026.

\[30\] ANTHROPIC. #emph[Claude --- project memory and CLAUDE.md.] Disponivel em: https:/\/docs.anthropic.com/. Acesso em: 4 ago. 2026.

\[31\] ANTHROPIC. #emph[Claude Code --- extensões, hooks e plugins.] Disponível em: https:/\/docs.anthropic.com/claude-code. Acesso em: 4 ago. 2026.

\[32\] ANTHROPIC. #emph[Claude Code hooks --- pre-commit, post-commit, tool call.] Disponível em: https:/\/docs.anthropic.com/claude-code/hooks. Acesso em: 4 ago. 2026.

\[33\] ANTHROPIC. #emph[Claude Code SDK --- programmatic agent integration.] Disponível em: https:/\/docs.anthropic.com/claude-code/sdk. Acesso em: 4 ago. 2026.

\[34\] ANTHROPIC. #emph[Claude Code --- skills and skill search.] Disponível em: https:/\/docs.anthropic.com/claude-code/skills. Acesso em: 4 ago. 2026.

\[35\] ANTHROPIC. #emph[Claude Code --- SKILL.md format and triggering.] Disponível em: https:/\/docs.anthropic.com/claude-code/skills. Acesso em: 4 ago. 2026.

\[36\] ANTHROPIC. #emph[Claude Code compose --- workflow orchestration.] Disponível em: https:/\/docs.anthropic.com/claude-code/compose. Acesso em: 4 ago. 2026.

\[37\] ANTHROPIC. #emph[Model Context Protocol (MCP) --- tool and resource providers.] Disponível em: https:/\/modelcontextprotocol.io/. Acesso em: 4 ago. 2026.

\[38\] ANTHROPIC. #emph[Claude Code Documentation --- tools, workflows, sub-agents and orchestration.] Disponível em: https:/\/docs.anthropic.com/en/docs/claude-code. Acesso em: 4 ago. 2026.

\[39\] ANTHROPIC. #emph[Claude Documentation --- multi-modal capabilities and tool use.] Disponível em: https:/\/docs.anthropic.com/en/docs. Acesso em: 4 ago. 2026.

\[40\] ANTHROPIC. #emph[Claude Privacy Policy --- data usage and retention.] Disponível em: https:/\/www.anthropic.com/privacy. Acesso em: 4 ago. 2026.

\[41\] ANTHROPIC. #emph[Model Context Protocol --- specification and documentation.] Disponível em: https:/\/modelcontextprotocol.io/. Acesso em: 4 ago. 2026.

\[42\] ANTHROPIC. #emph[MCP Servers --- official server implementations.] Disponível em: https:/\/github.com/modelcontextprotocol/servers. Acesso em: 4 ago. 2026.
...
\[45\] AUTOGEN. #emph[Microsoft AutoGen --- multi-agent conversations.] Disponivel em: https:/\/microsoft.github.io/autogen/. Acesso em: 4 ago. 2026.

\[46\] AZURE. #emph[Azure DevOps Pipelines --- multi-agent orchestration.] Disponível em: https:/\/learn.microsoft.com/en-us/azure/devops/pipelines/. Acesso em: 4 ago. 2026.

\[47\] BALA, Rajiv et al.~Containerized development environments: A systematic review. #emph[Journal of Systems and Software], v. 208, 2024. Disponível em: https:/\/doi.org/10.1016/j.jss.2023.111900. Acesso em: 15 jul. 2025.

\[48\] BOURNE, Stephen R. #emph[The Unix Programming Environment]. Upper Saddle River: Prentice Hall, 1984. 486 p.~ISBN 978-0-13-937724-9.

\[49\] BROHMAN, Katherine et al.~Designing effective AI assistants for software engineering. #emph[Proceedings of the 46th International Conference on Software Engineering], 2024. Disponível em: https:/\/dl.acm.org/doi/10.1145/3597503. Acesso em: 15 jul. 2025.

\[50\] BROOKS, Fred. #emph[The Mythical Man-Month: Essays on Software Engineering]. Anniversary ed.~Boston: Addison-Wesley, 1995. 322 p.~ISBN 978-0-201-83595-1.
...
\[52\] BROWN, Tom et al.~#emph[Language Models are Few-Shot Learners (GPT-3).] NeurIPS, 2020. Disponível em: https:/\/arxiv.org/abs/2005.14165. Acesso em: 4 ago. 2026.

\[53\] BURNETT, Margaret et al.~The interaction design of AI-assisted software development tools. #emph[Proceedings of the ACM on Human-Computer Interaction], v. 8, n.~CSCW1, 2024. Disponível em: https:/\/dl.acm.org/doi/10.1145/3637389. Acesso em: 15 jul. 2025.

\[54\] CHACON, Scott; STRAUB, Ben. #emph[Pro Git] (2nd ed.). Apress, 2014. Disponível em: https:/\/git-scm.com/book/en/v2. Acesso em: 4 ago. 2026.

\[55\] CHASE, Harrison. #emph[AI Engineering: Building Applications with LLMs and Agents.] O'Reilly Media, 2025.
...
\[57\] CHEN, Mark et al.~Evaluating large language models trained on code. #emph[arXiv preprint], 2021. Disponível em: https:/\/arxiv.org/abs/2107.03374. Acesso em: 15 jul. 2025.

\[58\] CNCF. #emph[Temporal --- durable execution engine.] Disponivel em: https:/\/temporal.io/. Acesso em: 4 ago. 2026.

\[59\] CNCF. #emph[Dapr --- state management building block.] Disponivel em: https:/\/docs.dapr.io/concepts/building-blocks/. Acesso em: 4 ago. 2026.

\[60\] CNCF. #emph[OpenTelemetry SDK --- extensibilidade e hooks.] Disponível em: https:/\/opentelemetry.io/docs/languages/sdk/. Acesso em: 4 ago. 2026.

\[61\] CNCF. #emph[OpenTelemetry --- semantic conventions and knowledge.] Disponível em: https:/\/opentelemetry.io/docs/concepts/semantic-conventions/. Acesso em: 4 ago. 2026.

\[62\] CREWAI. #emph[CrewAI --- agent orchestration framework.] Disponivel em: https:/\/docs.crewai.com/. Acesso em: 4 ago. 2026.

\[63\] CREWAI. #emph[CrewAI --- framework for orchestrating role-playing AI agents.] Disponível em: https:/\/docs.crewai.com/. Acesso em: 4 ago. 2026.

\[64\] CRON. #emph[Cron Wikipedia --- POSIX cron, crontab syntax and scheduling.] Disponível em: https:/\/en.wikipedia.org/wiki/Cron. Acesso em: 4 ago. 2026.

\[65\] CURSOR. #emph[Background Agents --- parallel AI coding.] Disponivel em: https:/\/cursor.sh/. Acesso em: 4 ago. 2026.

\[66\] DEEPMIND. #emph[Gemini 2.5 --- agentic capabilities and tool use.] Disponivel em: https:/\/deepmind.google/. Acesso em: 4 ago. 2026.
...
\[69\] DOCKER. #emph[Docker hooks --- pre-build and post-build.] Disponível em: https:/\/docs.docker.com/build/building/hooks/. Acesso em: 4 ago. 2026.

\[70\] DOCKER. #emph[Docker best practices --- production deployment.] Disponível em: https:/\/docs.docker.com/build/building/best-practices/. Acesso em: 4 ago. 2026.

\[71\] DOCKER. #emph[Docker Actions --- containerized CI/CD runners.] Disponível em: https:/\/docs.docker.com/ci-cd/. Acesso em: 4 ago. 2026.
...
\[74\] ERICSSON. #emph[Plugin Architecture --- patterns for extensible systems.] Disponível em: https:/\/www.ericsson.com/en/reports-and-papers/white-papers. Acesso em: 4 ago. 2026.

\[75\] ERLANG. #emph[OTP --- supervision trees and fault tolerance.] Disponivel em: https:/\/www.erlang.org/doc/apps/otp\_design/. Acesso em: 4 ago. 2026.

\[76\] ESLINT. #emph[ESLint --- pluggable linter for JavaScript/TypeScript.] Disponível em: https:/\/eslint.org/. Acesso em: 4 ago. 2026.

\[77\] EUROPEAN COMMISSION. #emph[EU AI Act --- Regulation on Artificial Intelligence.] Disponível em: https:/\/digital-strategy.ec.europa.eu/en/policies/regulatory-framework-ai. Acesso em: 4 ago. 2026.

\[78\] FIELDING, Roy T. #emph[Architectural Styles and the Design of Network-based Software Architectures]. Tese (Doutorado) --- University of California, Irvine, 2000. 180 p.

\[79\] FOWLER, Martin. #emph[Patterns of Enterprise Application Architecture]. Boston: Addison-Wesley, 2002. 533 p.~ISBN 978-0-321-12742-6.

\[80\] FOWLER, Martin. #emph[Refactoring: Improving the Design of Existing Code]. 2. ed.~Boston: Addison-Wesley, 2019. 434 p.~ISBN 978-0-13-475759-9.
...
\[84\] GIT. #emph[Documentation --- refs, stash and worktrees.] Disponivel em: https:/\/git-scm.com/docs/. Acesso em: 4 ago. 2026.

\[85\] GIT. #emph[Git Worktree Documentation --- multiple working trees.] Disponível em: https:/\/git-scm.com/docs/git-worktree. Acesso em: 4 ago. 2026.
...
\[88\] GITHUB. #emph[Copilot Workspace --- background agents.] Disponivel em: https:/\/github.com/features/copilot. Acesso em: 4 ago. 2026.

\[89\] GITHUB. #emph[Git hooks --- pre-commit and post-commit.] Disponível em: https:/\/git-scm.com/book/en/v2/Customizing-Git-Git-Hooks. Acesso em: 4 ago. 2026.

\[90\] GITHUB. #emph[GitHub Actions --- workflow hooks and custom actions.] Disponível em: https:/\/docs.github.com/en/actions. Acesso em: 4 ago. 2026.

\[91\] GITHUB. #emph[GitHub Actions --- reusable workflows.] Disponível em: https:/\/docs.github.com/en/actions/creating-actions/reusing-workflows. Acesso em: 4 ago. 2026.

\[92\] GITHUB. #emph[awesome-claude-code --- community extensions and skills.] Disponível em: https:/\/github.com/anthropics/awesome-claude-code. Acesso em: 4 ago. 2026.

\[93\] GITHUB. #emph[GitHub Actions Documentation --- workflows, triggers and reusable workflows.] Disponível em: https:/\/docs.github.com/en/actions. Acesso em: 4 ago. 2026.

\[94\] GITHUB. #emph[GitHub Actions Marketplace --- community-built actions.] Disponível em: https:/\/github.com/marketplace?type=actions. Acesso em: 4 ago. 2026.

\[95\] GITHUB. #emph[GitHub Copilot Documentation --- features and limitations.] Disponível em: https:/\/docs.github.com/en/copilot. Acesso em: 4 ago. 2026.

\[96\] GITLAB. #emph[GitLab CI/CD Documentation --- pipelines, jobs and hooks.] Disponível em: https:/\/docs.gitlab.com/ee/ci/. Acesso em: 4 ago. 2026.

\[97\] GOLDBERG, David. #emph[What Every Programmer Should Know About Memory]. 2. ed.~Upper Saddle River: Prentice Hall, 2009. 112 p.~ISBN 978-0-13-409266-5.

\[98\] GOOGLE. #emph[ADK (Agent Development Kit) --- agent orchestration.] Disponivel em: https:/\/google.github.io/adk-docs/. Acesso em: 4 ago. 2026.

\[99\] GOOGLE. #emph[Gemini context window and memory.] Disponivel em: https:/\/ai.google.dev/. Acesso em: 4 ago. 2026.

\[100\] GOOGLE. #emph[Gemini CLI --- extensions and customizations.] Disponível em: https:/\/github.com/google-gemini/gemini-cli. Acesso em: 4 ago. 2026.

\[101\] GOOGLE. #emph[Gemini CLI --- custom agents and extensions.] Disponível em: https:/\/github.com/google-gemini/gemini-cli. Acesso em: 4 ago. 2026.

\[102\] GRESHAKE, Kai et al.~#emph[Not What You've Signed Up For: Compromising Real-World LLM-Integrated Applications with Indirect Prompt Injection.] AISec '23, 2023. Disponível em: https:/\/arxiv.org/abs/2302.12173. Acesso em: 4 ago. 2026.

\[103\] GROSSKURTH, Alan et al.~API key management best practices. #emph[Journal of Cybersecurity], v. 10, n.~1, 2024. Disponível em: https:/\/academic.oup.com/cybersecurity/article/10/1/tyae013/7586573. Acesso em: 15 jul. 2025.
...
\[105\] HASHICORP. #emph[Terraform providers --- plugin architecture.] Disponível em: https:/\/developer.hashicorp.com/terraform/plugin. Acesso em: 4 ago. 2026.

\[106\] HASHICORP. #emph[Terraform modules --- reusable infrastructure knowledge.] Disponível em: https:/\/developer.hashicorp.com/terraform/language/modules. Acesso em: 4 ago. 2026.
...
\[108\] HOUGHTON, Andy. #emph[Git Worktrees: The Complete Guide.] 2023. Disponível em: https:/\/www.git-tower.com/learn/git/stashing/git-worktrees. Acesso em: 4 ago. 2026.

\[109\] HUMPHREYS, David. #emph[Managing Software Projects]. 2. ed.~Manchester: Europa Books, 2019. 346 p.~ISBN 978-1-912585-10-8.

\[110\] HUNT, Andrew; THOMAS, David. #emph[The Pragmatic Programmer: Your Journey to Mastery]. 2. ed.~Boston: Addison-Wesley, 2019. 352 p.~ISBN 978-0-13-595705-9.

\[111\] IEEE. #emph[Software Engineering --- extensibility and maintainability.] Disponível em: https:/\/ieeexplore.ieee.org/. Acesso em: 4 ago. 2026.

\[112\] IEEE. #emph[Software Engineering Body of Knowledge (SWEBOK) --- knowledge management.] Disponível em: https:/\/swebokwiki.org/. Acesso em: 4 ago. 2026.

\[113\] IETF. #emph[RFC 7252 --- The Constrained Application Protocol (CoAP).] Internet Engineering Task Force, 2014. Disponível em: https:/\/datatracker.ietf.org/doc/html/rfc7252. Acesso em: 4 ago. 2026.

\[114\] JENKINS. #emph[Jenkins Pipeline --- declarative and scripted pipelines.] Disponível em: https:/\/www.jenkins.io/doc/book/pipeline/. Acesso em: 4 ago. 2026.
...
\[116\] JIMENEZ, Carlos E. et al.~#emph[SWE-bench: Can Language Models Resolve Real-World GitHub Issues?] ICLR, 2024. Disponível em: https:/\/arxiv.org/abs/2310.06770. Acesso em: 4 ago. 2026.

\[117\] KELLY, Dan. The hidden costs of context switching in AI-assisted development. #emph[Communications of the ACM], v. 67, n.~4, 2024. Disponível em: https:/\/dl.acm.org/doi/10.1145/3643673. Acesso em: 15 jul. 2025.

\[118\] KERNIGHAN, Brian W.; RITCHIE, Dennis M. #emph[The C Programming Language]. 2. ed.~Upper Saddle River: Prentice Hall, 1988. 272 p.~ISBN 978-0-13-110362-7.
...
\[121\] KO, Andrew J. et al.~A field study of professional developers working with AI assistants. #emph[Proceedings of the IEEE International Conference on Software Maintenance and Evolution], 2024. Disponível em: https:/\/ieeexplore.ieee.org/document/10636470. Acesso em: 15 jul. 2025.

\[122\] KOZLOV, Dmitry. #emph[Self-Healing CI/CD Pipelines with AI Agents.] arXiv, 2025. Disponível em: https:/\/arxiv.org/abs/2501.04523. Acesso em: 4 ago. 2026.

\[123\] KRUG, Steve. #emph[Don't Make Me Think, Revisited: A Common Sense Approach to Web Usability]. 3. ed.~San Francisco: New Riders, 2014. 200 p.~ISBN 978-0-321-96551-6.

\[124\] KUBERNETES. #emph[Dynamic admission control --- webhooks.] Disponível em: https:/\/kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/. Acesso em: 4 ago. 2026.

\[125\] KUBERNETES. #emph[Kubernetes documentation --- cluster management.] Disponível em: https:/\/kubernetes.io/docs/. Acesso em: 4 ago. 2026.

\[126\] LAKOFF, George; JOHNSON, Mark. #emph[Metaphors We Live By]. Chicago: University of Chicago Press, 1980. 242 p.~ISBN 978-0-226-46801-3.

\[127\] LANGCHAIN. #emph[LangGraph --- multi-agent orchestration patterns.] Disponivel em: https:/\/langchain-ai.github.io/langgraph/. Acesso em: 4 ago. 2026.

\[128\] LANGCHAIN. #emph[LangGraph checkpoint persistence.] Disponivel em: https:/\/langchain-ai.github.io/langgraph/. Acesso em: 4 ago. 2026.

\[129\] LANGCHAIN. #emph[LangSmith --- LLM application observability.] Disponível em: https:/\/docs.smith.langchain.com/. Acesso em: 4 ago. 2026.

\[130\] LEWIS, P. et al.~#emph[Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks.] NeurIPS, 2020. Disponível em: https:/\/arxiv.org/abs/2005.11401. Acesso em: 4 ago. 2026.

\[131\] LEWIS, Patrick et al.~#emph[Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks.] NeurIPS, 2020. Disponível em: https:/\/arxiv.org/abs/2005.11401. Acesso em: 4 ago. 2026.

\[132\] LI, Junlin et al.~Context-aware code generation for software development: A survey. #emph[ACM Computing Surveys], v. 56, n.~9, 2024. Disponível em: https:/\/arxiv.org/abs/2405.01453. Acesso em: 15 jul. 2025.

\[133\] LI, Raymond et al.~#emph[AlphaCode 2: Large Language Model Coding with DeepMind.] arXiv, 2024. Disponível em: https:/\/arxiv.org/abs/2401.14196. Acesso em: 4 ago. 2026.
...
\[136\] LOVERING, Cameron. Shell aliases best practices. #emph[Linux Journal], 2023. Disponível em: https:/\/linuxjournal.com/article/shell-aliases-best-practices. Acesso em: 15 jul. 2025.

\[137\] LUTHER, Kurt et al.~Secrets in the cloud: A study of credential management practices. #emph[Proceedings of the ACM CCS], 2024. Disponível em: https:/\/dl.acm.org/doi/10.1145/3658644.3670300. Acesso em: 15 jul. 2025.

\[138\] MARR, Bernard. The four generations of AI coding assistants. #emph[Forbes], 2024. Disponível em: https:/\/www.forbes.com/sites/bernardmarr/2024/03/25/the-four-generations-of-ai-coding-assistants/. Acesso em: 15 jul. 2025.

\[139\] MAYER, Richard E. #emph[Multimedia Learning]. 3. ed.~Cambridge: Cambridge University Press, 2021. 424 p.~ISBN 978-1-108-83905-2.

\[140\] McGOWAN, Vince. Secure handling of secrets in development environments. #emph[Proceedings of the ACM Conference on Computer and Communications Security], 2023. Disponível em: https:/\/dl.acm.org/doi/10.1145/3576915.3623144. Acesso em: 15 jul. 2025.

\[141\] MCKEE, Patrick. #emph[Automation Anti-Patterns in Software Engineering.] ACM Queue, v. 22, n.~1, 2024.
...
\[144\] MICROSOFT. #emph[VS Code extension architecture.] Disponível em: https:/\/code.visualstudio.com/api/extension-guides/overview. Acesso em: 4 ago. 2026.

\[145\] MICROSOFT. #emph[VS Code extension marketplace --- knowledge sharing.] Disponível em: https:/\/marketplace.visualstudio.com/vscode. Acesso em: 4 ago. 2026.

\[146\] MICROSOFT. #emph[AutoGen --- multi-agent conversation framework.] Disponível em: https:/\/microsoft.github.io/autogen/. Acesso em: 4 ago. 2026.
...
\[150\] MIMOCODE. #emph[Documentacao oficial --- task tool (planejamento persistente).] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[151\] MIMOCODE. #emph[Documentacao oficial --- actor tool (orquestacao de sub-agentes).] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[152\] MIMOCODE. #emph[Guia de uso --- sub-agentes, context inheritance e spawn.] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[153\] MIMOCODE. #emph[Agentes nativos --- explore e general ( tipos e tool allowlists).] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[154\] MIMOCODE. #emph[Gerenciamento de contexto --- compaction, overflow e context inheritance.] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[155\] MIMOCODE. #emph[Documentacao oficial --- memoria, sessoes e checkpoints.] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[156\] MIMOCODE. #emph[Gerenciamento de contexto --- compaction, overflow, pruning.] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[157\] MIMOCODE. #emph[MEMORY.md e memoria persistente --- estrutura e busca.] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[158\] MIMOCODE. #emph[Checkpoint-writer --- sub-agente fork de snapshots periodicos.] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[159\] MIMOCODE. #emph[Busca por memoria --- BM25, scope e queries.] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[160\] MIMOCODE. #emph[Sessoes --- lifecycle, continue, resume e session-dir.] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[161\] MIMOCODE. #emph[Dream e Distill --- aprendizado entre sessoes.] Disponivel em: https:/\/mimo.xiaomi.com/mimocode/. Acesso em: 4 ago. 2026.

\[162\] MODEL CONTEXT PROTOCOL. #emph[Especificacao MCP --- tool use e resources.] Disponivel em: https:/\/spec.modelcontextprotocol.io/. Acesso em: 4 ago. 2026.

\[163\] MOSQUITTO. #emph[Eclipse Mosquitto --- configuration and security.] Disponível em: https:/\/mosquitto.org/documentation/. Acesso em: 4 ago. 2026.

\[164\] MYERS, Glenford J.; SANDLER, Corey; BADGETT, Tom. #emph[The Art of Software Testing]. 3. ed.~Hoboken: John Wiley & Sons, 2011. 256 p.~ISBN 978-1-118-03196-0.

\[165\] NEWMAN, Sam. #emph[Building Microservices: Designing Fine-Grained Systems]. 2. ed.~Sebastopol: O'Reilly Media, 2021. 612 p.~ISBN 978-1-4920-3402-5.

\[166\] NIELSEN, Jakob. #emph[Usability Engineering]. San Francisco: Morgan Kaufmann, 1994. 358 p.~ISBN 978-0-12-518406-0.

\[167\] NIERENBERG, Dale. Practical secrets management for development teams. #emph[Proceedings of the USENIX Security Symposium], 2023. Disponível em: https:/\/www.usenix.org/conference/usenixsecurity23/presentation/niernenberg. Acesso em: 15 jul. 2025.

\[168\] NIST. #emph[AI Risk Management Framework --- SP 1270.] Disponível em: https:/\/www.nist.gov/artificial-intelligence/risk-management-framework. Acesso em: 4 ago. 2026.

\[169\] NODE-RED. #emph[Node-RED --- flow-based programming for IoT.] Disponível em: https:/\/nodered.org/. Acesso em: 4 ago. 2026.

\[170\] NODE.JS. #emph[Node.js Test Runner --- built-in testing with CI hooks.] Disponível em: https:/\/nodejs.org/docs/latest/api/test.html. Acesso em: 4 ago. 2026.
...
\[173\] NOMURA, Tatsuki et al.~#emph[AutoAgent: Fully Autonomous Framework for LLM-Based Agents.] arXiv, 2025. Disponível em: https:/\/arxiv.org/abs/2502.05907. Acesso em: 4 ago. 2026.

\[174\] NORTON, Q. The art of the prompt: how to communicate with AI effectively. O'Reilly Media, 2024. ISBN 978-1-098-15343-2.

\[175\] NPM. #emph[ESLint plugins --- arquitetura de extensão.] Disponível em: https:/\/eslint.org/docs/latest/extend/plugins. Acesso em: 4 ago. 2026.

\[176\] NVIDIA. #emph[NemoGuardrails --- agent safety and orchestration.] Disponivel em: https:/\/github.com/NVIDIA/NeMo-Guardrails. Acesso em: 4 ago. 2026.
...
\[190\] OH-MY-PI. #emph[Oh My Pi Documentation --- cron tool, hooks, workflows.] Disponível em: https:/\/ohmypi.dev/docs. Acesso em: 4 ago. 2026.

\[191\] OH-MY-PI. #emph[Self-learning skill --- capturing golden paths.] Disponível em: https:/\/ohmypi.dev/docs/skills/self-learning. Acesso em: 4 ago. 2026.
...
\[195\] OPENAI. #emph[Agents SDK --- handoffs and parallel execution.] Disponivel em: https:/\/openai.github.io/openai-agents-python/. Acesso em: 4 ago. 2026.

\[196\] OPENAI. #emph[ChatGPT memory --- persistent memory across sessions.] Disponivel em: https:/\/openai.com/. Acesso em: 4 ago. 2026.

\[197\] OPENAI. #emph[Codex CLI --- configuration and extensions.] Disponível em: https:/\/github.com/openai/codex. Acesso em: 4 ago. 2026.

\[198\] OPENAI. #emph[Codex CLI --- custom agents and configurations.] Disponível em: https:/\/github.com/openai/codex. Acesso em: 4 ago. 2026.

\[199\] OPENAI. #emph[GPT-4o Technical Report --- multi-modal architecture.] Disponível em: https:/\/openai.com/index/hello-gpt-4o/. Acesso em: 4 ago. 2026.

\[200\] OPENAI. #emph[Agents SDK --- multi-agent orchestration framework.] Disponível em: https:/\/openai.github.io/openai-agents-python/. Acesso em: 4 ago. 2026.

\[201\] OWASP. #emph[OWASP Top 10 for LLM Applications --- LLM07: Insecure Plugin Design.] Disponível em: https:/\/owasp.org/www-project-top-10-for-large-language-model-applications/. Acesso em: 4 ago. 2026.
...
\[203\] PARNAS, David L. Designing software for ease of extension and contraction. #emph[IEEE Transactions on Software Engineering], v. SE-5, n.~2, p.~128-138, 1979. Disponível em: https:/\/ieeexplore.ieee.org/document/4393556. Acesso em: 15 jul. 2025.

\[204\] PARNAS, David L. The dangers of AI-generated code. #emph[Communications of the ACM], v. 67, n.~2, 2024. Disponível em: https:/\/dl.acm.org/doi/10.1145/3639481. Acesso em: 15 jul. 2025.

\[205\] PARRISH, Nate. AI coding agents in CI/CD pipelines. #emph[The New Stack], 2024. Disponível em: https:/\/thenewstack.io/ai-coding-agents-in-ci-cd-pipelines/. Acesso em: 15 jul. 2025.

\[206\] PAUL, Robert. #emph[Crontab Guru --- cron expression editor and validator.] Disponível em: https:/\/crontab.guru/. Acesso em: 4 ago. 2026.

\[207\] PERFORM. #emph[Adversarial Attacks on LLM-Integrated Applications: A Survey.] ACM Computing Surveys, 2024. Disponível em: https:/\/arxiv.org/abs/2312.07693. Acesso em: 4 ago. 2026.

\[208\] POSTGRES. #emph[Write-ahead logging (WAL) --- durability guarantees.] Disponivel em: https:/\/www.postgresql.org/docs/current/wal-intro.html. Acesso em: 4 ago. 2026.

\[209\] PRETTIER. #emph[Prettier --- code formatter with editor hooks.] Disponível em: https:/\/prettier.io/. Acesso em: 4 ago. 2026.

\[210\] PYPI. #emph[pre-commit framework --- gerenciador de hooks.] Disponível em: https:/\/pre-commit.com/. Acesso em: 4 ago. 2026.

\[211\] PYPI. #emph[paho-mqtt --- Python MQTT client library.] Disponível em: https:/\/pypi.org/project/paho-mqtt/. Acesso em: 4 ago. 2026.

\[212\] PYTHON. #emph[pip plugins --- setuptools and importlib.metadata.] Disponível em: https:/\/setuptools.pypa.io/. Acesso em: 4 ago. 2026.

\[213\] QIAN, Chen et al.~#emph[ChatDev: Communicative Agents for Software Development.] ACL, 2024. Disponível em: https:/\/arxiv.org/abs/2307.07924. Acesso em: 4 ago. 2026.
...
\[216\] RASPBERRY PI. #emph[Documentation --- SSH remote access.] Disponível em: https:/\/www.raspberrypi.com/documentation/computers/remote-access.html. Acesso em: 4 ago. 2026.

\[217\] RASPBERRY PI. #emph[Raspberry Pi OS --- system configuration (raspi-config).] Disponível em: https:/\/www.raspberrypi.com/documentation/computers/configuration.html. Acesso em: 4 ago. 2026.

\[218\] RASPBERRY PI. #emph[Documentation --- Raspberry Pi OS configuration.] Disponível em: https:/\/www.raspberrypi.com/documentation/computers/configuration.html. Acesso em: 4 ago. 2026.

\[219\] RASPBERRY PI. #emph[Compute Module --- industrial deployment guide.] Disponível em: https:/\/www.raspberrypi.com/documentation/computers/compute-module.html. Acesso em: 4 ago. 2026.

\[220\] RASPBERRY PI. #emph[GitHub Actions for Raspberry Pi --- community workflows.] Disponível em: https:/\/github.com/raspberrypi/actions. Acesso em: 4 ago. 2026.

\[221\] RAYMOND, Eric S. #emph[The Art of Unix Programming]. Boston: Addison-Wesley, 2003. 528 p.~ISBN 978-0-13-142901-7.

\[222\] RAYMOND, Eric S. #emph[The Cathedral and the Bazaar: Musings on Linux and Open Source by an Accidental Revolutionary]. 2. ed.~Sebastopol: O'Reilly Media, 2001. 292 p.~ISBN 978-0-596-00108-7.

\[223\] REDIS. #emph[Redis Streams --- persistent message history.] Disponivel em: https:/\/redis.io/docs/streams/. Acesso em: 4 ago. 2026.

\[224\] RICHARDS, Matt. Building effective agents. #emph[Anthropic Engineering], 2024. Disponível em: https:/\/www.anthropic.com/engineering/building-effective-agents. Acesso em: 15 jul. 2025.

\[225\] ROBERTSON, S. E.; ZARAGOZA, H. #emph[The Probabilistic Relevance Framework: BM25 and Beyond.] Foundations and Trends in Information Retrieval, v. 3, n.~4, 2009. Disponível em: https:/\/doi.org/10.1561/1500000006. Acesso em: 4 ago. 2026.

\[226\] ROTHMAN, Daniel. The terminal Renaissance: Why CLI tools are making a comeback. #emph[InfoWorld], 2024. Disponível em: https:/\/www.infoworld.com/article/terminal-renaissance-cli-tools-comeback.html. Acesso em: 15 jul. 2025.

\[227\] RUST. #emph[Cargo plugins --- extensibilidade do gerenciador de pacotes.] Disponível em: https:/\/doc.rust-lang.org/cargo/. Acesso em: 4 ago. 2026.

\[228\] RUST. #emph[Cargo --- Rust package manager with CI integration.] Disponível em: https:/\/doc.rust-lang.org/cargo/. Acesso em: 4 ago. 2026.

\[229\] SCHICK, Timo et al.~Toolformer: Language models can teach themselves to use tools. #emph[Advances in Neural Information Processing Systems], v. 36, 2023. Disponível em: https:/\/arxiv.org/abs/2302.04761. Acesso em: 15 jul. 2025.

\[230\] SCHNEIER, Bruce. #emph[Secrets and Lies: Digital Security in a Networked World]. Hoboken: John Wiley & Sons, 2015. 432 p.~ISBN 978-1-119-09278-0.
...
\[232\] SHNEIDERMAN, Ben. #emph[Designing the User Interface: Strategies for Effective Human-Computer Interaction]. 6. ed.~Hoboken: Pearson, 2017. 612 p.~ISBN 978-0-13-438036-4.

\[233\] SHNEIDERMAN, Ben. #emph[Human-Computer Interaction: An Empirical Research Perspective]. 2. ed.~Boca Raton: CRC Press, 2022. 458 p.~ISBN 978-1-138-07020-0.

\[234\] SQLITE. #emph[Documentation --- persistent storage engine.] Disponivel em: https:/\/www.sqlite.org/docs.html. Acesso em: 4 ago. 2026.

\[235\] STALLINGS, William. #emph[Operating Systems: Internals and Design Principles]. 9. ed.~Hoboken: Pearson, 2018. 816 p.~ISBN 978-0-13-467095-2.

\[236\] STALLMAN, Richard. #emph[Free Software, Free Society: Selected Essays of Richard M. Stallman]. 2. ed.~Boston: GNU Press, 2015. 464 p.~ISBN 978-0-9831592-4-7.

\[237\] STANFORD. #emph[Computer Science --- Software Architecture and Design Patterns.] Disponível em: https:/\/cs.stanford.edu/people/nick/how-software-is-built/. Acesso em: 4 ago. 2026.

\[238\] STEEL, Daniel et al.~The human in the loop: Effective collaboration with AI coding agents. #emph[Proceedings of CHI], 2024. Disponível em: https:/\/dl.acm.org/doi/10.1145/3613904.3642596. Acesso em: 15 jul. 2025.

\[239\] TANG, J. et al.~Large language models as coding assistants: A survey. ACM Computing Surveys, v. 57, n.~3, p.~1--42, 2025.

\[240\] TEMPORAL. #emph[Durable execution --- state persistence across failures.] Disponivel em: https:/\/temporal.io/. Acesso em: 4 ago. 2026.
...
\[242\] THIRUVALLUVAR, Karthikeyan et al.~The case for open protocols in AI agent ecosystems. #emph[Proceedings of the ACM Workshop on AI Agent Ecosystems], 2024. Disponível em: https:/\/dl.acm.org/doi/10.1145/3643680. Acesso em: 15 jul. 2025.

\[243\] THOMPSON, Clive. #emph[Smarter Than You Think: How Technology Is Changing Our Minds for the Better]. New York: Penguin Press, 2013. 356 p.~ISBN 978-1-59420-445-6.

\[244\] TORVALDS, Linus; DIACONESCU, Greg. #emph[Just for Fun: The Story of an Accidental Revolutionary]. New York: HarperBusiness, 2002. 272 p.~ISBN 978-0-06-662073-3.

\[245\] TRUSS, Ben et al.~Effective environment management for AI-assisted development. #emph[IEEE Software], v. 41, n.~5, 2024. Disponível em: https:/\/ieeexplore.ieee.org/document/10547890. Acesso em: 15 jul. 2025.

\[246\] UNIX. #emph[fork() e processos paralelos --- manual de referência.] Disponivel em: https:/\/pubs.opengroup.org/onlinepubs/9699919799/. Acesso em: 4 ago. 2026.

\[247\] VALENTIM, Bruna et al.~Code quality in the era of AI assistants: An empirical study. #emph[Journal of Systems and Software], v. 205, 2024. Disponível em: https:/\/doi.org/10.1016/j.jss.2023.111808. Acesso em: 15 jul. 2025.

\[248\] VAN ROSSUM, G.; DRAGON, F. Python reference manual. Centrum Wiskunde & Informatica, 1995.

\[249\] VERCEL. #emph[Vercel Workflow Engine --- documentation and examples.] Disponível em: https:/\/vercel.com/docs/workflow-collaboration. Acesso em: 4 ago. 2026.

\[250\] VIM. #emph[Plugin architecture --- runtime path.] Disponível em: https:/\/vimhelp.org/repeat.txt.html\#runtime. Acesso em: 4 ago. 2026.

\[251\] WANG, Lei et al.~A survey on large language model based autonomous agents. #emph[arXiv preprint], 2023. Disponível em: https:/\/arxiv.org/abs/2308.11432. Acesso em: 15 jul. 2025.

\[252\] WANG, Menghan et al.~#emph[The Rise of AI Engineers: A Survey on LLM-Based Software Engineering.] arXiv, 2025. Disponível em: https:/\/arxiv.org/abs/2501.02780. Acesso em: 4 ago. 2026.
...
\[255\] WIKIPEDIA. #emph[Actor model --- Carl Hewitt, 1973.] Disponivel em: https:/\/en.wikipedia.org/wiki/Actor\_model. Acesso em: 4 ago. 2026.

\[256\] WIKIPEDIA. #emph[Information retrieval --- BM25 ranking function.] Disponivel em: https:/\/en.wikipedia.org/wiki/Okapi\_BM25. Acesso em: 4 ago. 2026.

\[257\] WIKIPEDIA. #emph[Experience replay --- reinforcement learning.] Disponivel em: https:/\/en.wikipedia.org/wiki/Experience\_replay. Acesso em: 4 ago. 2026.

\[258\] WILKINSON, Matt; BOSNIC, Niko. #emph[CI/CD Pipelines with AI Agents: Patterns and Anti-Patterns.] IEEE Software, v. 41, n.~3, 2024. Disponível em: https:/\/doi.org/10.1109/MS.2024.3357812. Acesso em: 4 ago. 2026.

\[259\] WING, Jeannette M. Computational thinking. #emph[Communications of the ACM], v. 49, n.~3, p.~33-35, 2006. Disponível em: https:/\/dl.acm.org/doi/10.1145/1118178.1118215. Acesso em: 15 jul. 2025.

\[260\] WIRTH, Niklaus. #emph[Programming in Modula-2]. 3. ed.~Berlin: Springer-Verlag, 1988. 298 p.~ISBN 978-3-540-18224-7.
...
\[263\] YANG, John et al.~#emph[AutoCodeRover: Program Improvement with Autonomous Agents.] arXiv, 2024. Disponível em: https:/\/arxiv.org/abs/2404.05427. Acesso em: 4 ago. 2026.

\[264\] YAO, Shunyu et al.~ReAct: Synergizing reasoning and acting in language models. #emph[Proceedings of ICLR], 2023. Disponível em: https:/\/arxiv.org/abs/2210.03629. Acesso em: 15 jul. 2025.
...
\[267\] ZHAO, Weixuan et al.~A survey on large language model based autonomous agents. #emph[Frontiers of Computer Science], v. 18, n.~6, 2024. Disponível em: https:/\/arxiv.org/abs/2308.11432. Acesso em: 15 jul. 2025.

\[268\] ZHENG, Boyang et al.~#emph[OpenDevin: An Open Platform for AI Software Developers as Generalist Agents.] arXiv, 2024. Disponível em: https:/\/arxiv.org/abs/2407.16741. Acesso em: 4 ago. 2026.
...
\[270\] ZHENG, Lianmin et al.~Judging LLM-as-a-Judge with MT-Bench and Chatbot Arena. #emph[Advances in Neural Information Processing Systems], v. 36, 2023. Disponível em: https:/\/arxiv.org/abs/2306.05685. Acesso em: 15 jul. 2025.
...
\[273\] ZIEGLER, Alex et al.~Productivity assessment of neural code completion. #emph[Proceedings of the 6th ACM SIGPLAN International Symposium on Machine Programming], 2022. Disponível em: https:/\/dl.acm.org/doi/10.1145/3520312.3520332. Acesso em: 15 jul. 2025.

\[274\] ZUCCHELLI, Stefano. #emph[Git Worktrees for Parallel Development.] Practical Git, Apress, 2022.

#heading(level: 1, outlined: false, numbering: none)[Contracapa]
<contracapa>
#strong[Oh My Pi: Do Zero ao PhD em Coding Agents]
...
// ── CONTRACAPA ────────────────────────────────────────────────────
  ...
  page(fill: cor.primaria, margin: 0cm, header: none, footer: none, numbering: none)[
    ...
    #place(top + left, dx: 2.5cm, dy: 4cm, block(width: 14.5cm)[
      #text(size: 18pt, weight: "bold", fill: cor.destaque)[Oh My Pi: Do Zero ao PhD em Coding Agents]
      #v(1cm)
      #text(size: 11.5pt, fill: white)[Abertura de impacto: a revoluÃ§Ã£o dos coding agents - como ferramentas de IA estÃ£o transformando a forma como programamos. Apresenta o Oh My Pi como um agente moderno que combina terminal, IDE e inteligÃªncia artificial. O arco da obra: levar o leitor do primeiro comando atÃ© dominar sub-agentes, plugins e automaÃ§Ã£o avanÃ§ada.]
      #v(1.2cm)
