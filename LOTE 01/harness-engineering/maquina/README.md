# Máquina de Vendas — Harness Engineering

Máquina de vendas da coleção **Harness Engineering — Do Modelo ao Sistema Autônomo Confiável**: captura, nutrição e conversão de leads do nicho de engenharia de software e agentes de IA.
...
# Dry run (sem salvar)
python scripts/lead_hunter.py --hashtags livro --dry-run
```
...
# Filtrar por persona
python scripts/email_sender.py --funil nutricao-livro --inscrever --persona aspirante-autor
```
...
python scripts/auto_correct.py criar \
  --slug test-assunto-livro \
  --nome "Teste Assunto E-mail Livro" \
  --variante-a "Seu livro está pronto!" \
...
Copy personalizada por nicho aplicada (engenharia de software e agentes de IA).
