...
<o-primeiro-contato-com-ia-chat-simples>
=== 1.1 Introdução
<introdução>
Este capítulo é o ponto de partida da nossa jornada. Aqui, você vai
configurar seu ambiente de desenvolvimento, entender o fluxo básico de
uma chamada à API de Inteligência Artificial, e construir seu primeiro
chat funcional --- um assistente que responde perguntas sobre um
documento específico.

#strong[O que você vai construir:]

Um script Python simples que: - Conecta à API do DeepSeek (ou OpenAI) -
Recebe uma pergunta do usuário - Retorna uma resposta gerada por IA -
Historico de conversas em memória

#strong[Por que este projeto importa:]

Cada grande sistema de IA começa com uma chamada simples. Antes de
arquiteturas complexas com RAG, fine-tuning e milhares de usuários,
existe uma única chamada de API que funciona. Este capítulo garante que
essa chamada funcione perfeitamente antes de construir sobre ela.

#strong[Requisitos prévios:] - Python 3.11+ instalado - Conta em uma
plataforma de API de IA (DeepSeek ou OpenAI) - Editor de código (VS Code
recomendado) - Conhecimento básico de Python (variáveis, funções, loops)

=== 1.2 Explica
<explica>
==== O que é Inteligência Artificial Generativa
<o-que-é-inteligência-artificial-generativa>
Inteligência Artificial Generativa refere-se a sistemas capazes de criar
conteúdo novo --- texto, imagens, código, música --- baseado em padrões
aprendidos a partir de grandes volumes de dados \[1\]. Diferente da IA
tradicional que classifica ou prevê, a IA generativa #strong[produz].

Os modelos de linguagem grandes (LLMs --- Large Language Models) são o
tipo mais comum de IA generativa hoje. Um LLM é treinado em bilhões de
tokens (pedaços de texto) e aprende a prever a próxima palavra em uma
sequência \[2\]. Quando você pergunta "O que é Python?", o modelo não
"busca" uma resposta em um banco de dados --- ele #strong[gera] uma
resposta baseada nos padrões que aprendeu durante o treinamento.

#strong[Analogia:] Imagine um escritor extremamente bem-leido que leu
milhões de livros. Quando você pergunta algo, ele não consulta uma
enciclopédia --- ele escreve uma resposta baseado em tudo que já leu. É
assim que um LLM funciona.

==== Como LLMs Funcionam (Visão Geral)
<como-llms-funcionam-visão-geral>
Um LLM opera em três etapas principais \[3\]:

+ #strong[Tokenização:] O texto de entrada é dividido em tokens
  (palavras ou sub-palavras). "Inteligência Artificial" pode virar
  \["Intelig", "ência", " Artificial"\].

+ #strong[Processamento:] Cada token passa através de camadas de rede
  neural (transformers) que calculam relações entre todas as palavras da
  sequência. O mecanismo de atenção (attention) permite que o modelo
  entenda contexto --- por exemplo, que "ele" no início de uma frase
  pode referir-se a uma pessoa mencionada no parágrafo anterior \[4\].

+ #strong[Geração:] O modelo gera um token por vez, calculando a
  probabilidade de cada palavra possível no vocabulário e escolhendo a
  mais provável (ou uma amostrada de acordo com parâmetros como
  "temperatura").

#strong[Parâmetros-chave que você vai usar:]

#figure(
  align(center)[#table(
    columns: (28.21%, 28.21%, 43.59%),
    align: (auto,auto,auto,),
    table.header([Parâmetro], [O que faz], [Valores típicos],),
    table.hline(),
    [`temperature`], [Controla criatividade/aleatoriedade], [0.0
    (determinístico) a 1.0 (criativo)],
...
<o-fluxo-de-uma-chamada-à-api>
Quando seu código envia uma requisição à API de IA, acontece o seguinte
\[5\]:
...
+ Seu Python monta um payload JSON com a mensagem e parâmetros
+ Uma requisição HTTP POST é enviada ao endpoint da API
+ O servidor autentica (verifica sua API key)
+ O payload é roteado para um servidor com GPU que hospeda o modelo
+ O modelo processa os tokens e gera uma resposta
+ A resposta retorna ao seu código como JSON
+ Seu Python extrai o texto da resposta

#strong[Tempo típico:] 0.5-3 segundos para respostas curtas, dependendo
do modelo e latência da rede.

==== Por que DeepSeek?
<por-que-deepseek>
O DeepSeek é uma empresa chinesa de IA que lançou modelos de alta
performance com preços competitivos \[6\]. Seus principais modelos
incluem:

- #strong[DeepSeek-V4-Flash:] Rápido e barato, ideal para tarefas
  simples
- #strong[DeepSeek-V4-Pro:] Mais preciso, melhor para tarefas complexas
- #strong[DeepSeek-R1:] Modelo de raciocínio (thinking model)

Para este livro, usaremos o DeepSeek-V4-Flash como modelo principal
(custo baixo) e o V4-Pro para tarefas que exigem mais qualidade.

#strong[Vantagens do DeepSeek para iniciantes:] - Preço acessível
(\$0.27/milhão de tokens de entrada para V4-Flash) - Compatível com a
API do OpenAI (mesma biblioteca) - Bom desempenho em português -
...
<estrutura-do-projeto>
Antes de escrever mais código, vamos organizar o projeto. Uma boa
estrutura facilita manutenção e expansão:
...
#strong[Por que essa estrutura?]

- #strong[Separação de responsabilidades:] Cada arquivo faz uma coisa
- #strong[Testabilidade:] Código isolado é mais fácil de testar
- #strong[Escalabilidade:] Fácil de adicionar novos componentes (RAG,
  auth, etc.)
- #strong[Padrão da indústria:] Segue convenções que outros
  desenvolvedores reconhecem
...
<exercício-prático-1-setup-completo>
+ #strong[Clone ou crie a pasta do projeto:]
...
+ #strong[Instale as dependências:]
]
...
+ #strong[Configure as variáveis de ambiente:]
]
...
<exercício-prático-2-variações-do-system-prompt>
Modifique o `SYSTEM_PROMPT` em `main.py` e observe como o comportamento
muda:

#strong[Versão Professor:]
...
- ☐ Histórico de mensagens funciona (o assistente lembra de mensagens
  anteriores)
...
=== 1.6 Conclusão
<conclusão>
Neste capítulo, você configurou seu ambiente, entendeu o fluxo básico de
uma chamada à IA, e construiu um chat funcional. O projeto agora tem:

- #strong[Cliente da API] com tratamento de erros e retry
- #strong[Chat com histórico] que mantém contexto
- #strong[System prompt] configurável
- #strong[Estrutura de projeto] pronta para expansão

No próximo capítulo, vamos adicionar persistência ao chat --- salvando
conversas em um banco de dados --- e criar uma API REST para que outros
aplicativos possam se comunicar com nosso assistente.

#strong[Lembre-se:] Todo grande sistema começa com uma chamada simples
que funciona. Agora que essa chamada funciona, podemos construir sobre
ela com confiança.

=== 1.7 Referências
<referências>
\[1\] Microsoft Azure Architecture Center. "Get Started with AI
Architecture Design." Microsoft Learn, 2024. Disponível em:
https:/\/learn.microsoft.com/en-us/azure/architecture/ai-ml/ai-get-started

\[2\] Vaswani, A. et al.~"Attention Is All You Need." Advances in Neural
Information Processing Systems, vol.~30, 2017.

\[3\] Brown, T. et al.~"Language Models are Few-Shot Learners." Advances
...
\[5\] OpenAI. "API Reference --- Chat Completions." OpenAI Platform
Documentation, 2024. Disponível em:
https:/\/platform.openai.com/docs/api-reference/chat

\[6\] DeepSeek. "API Documentation --- Model Overview." DeepSeek API
Docs, 2024. Disponível em:
https:/\/api-docs.deepseek.com/guides/model\_overview

\[7\] Huyen, Chip. "Designing Machine Learning Systems." O'Reilly Media,
\2022. ISBN: 978-1098107963.

\[8\] AWS. "Machine Learning Lens --- Well-Architected Framework."
Amazon Web Services, 2024. Disponível em:
https:/\/docs.aws.amazon.com/wellarchitected/latest/machine-learning-lens/

\[9\] Google Cloud. "ML System Design Patterns." Google Cloud
Architecture Center, 2023. Disponível em:
https:/\/cloud.google.com/architecture/ml-design-patterns

\[10\] Pinecone. "What is RAG?" Pinecone Learning Center, 2024.
Disponível em:
https:/\/www.pinecone.io/learn/retrieval-augmented-generation/

\[11\] LangChain. "RAG from Scratch." LangChain Documentation, 2024.
Disponível em: https:/\/python.langchain.com/docs/tutorials/rag/

\[12\] DeepSeek. "Pricing --- API." DeepSeek API Docs, 2024. Disponível
em: https:/\/api-docs.deepseek.com/quick\_start/pricing

\[13\] OWASP. "Top 10 for Large Language Model Applications." OWASP
Foundation, 2024. Disponível em:
https:/\/owasp.org/www-project-top-10-for-large-language-model-applications/

\[14\] NIST. "Artificial Intelligence Risk Management Framework."
National Institute of Standards and Technology, 2024. Disponível em:
https:/\/www.nist.gov/artificial-intelligence/risk-management-framework

\[15\] Docker. "Containerization Best Practices." Docker Documentation,
\2024. Disponível em: https:/\/docs.docker.com/

\[16\] FastAPI. "Production Deployment." FastAPI Documentation, 2024.
Disponível em: https:/\/fastapi.tiangolo.com/deployment/

\[17\] Prometheus. "Monitoring Best Practices." Prometheus
Documentation, 2024. Disponível em: https:/\/prometheus.io/docs/

\[18\] Grafana. "Dashboard Design." Grafana Documentation, 2024.
Disponível em: https:/\/grafana.com/docs/

\[19\] Hugging Face. "PEFT Library --- Parameter-Efficient Fine-Tuning."
Hugging Face Documentation, 2024. Disponível em:
https:/\/huggingface.co/docs/peft

\[20\] DeepEval. "LLM Evaluation Framework." DeepEval Documentation,
\2024. Disponível em: https:/\/docs.confident-ai.com/

==== Debug e Troubleshooting Comum
<debug-e-troubleshooting-comum>
Ao desenvolver com APIs de IA, você vai encontrar problemas comuns. Aqui
estão os mais frequentes e como resolvê-los \[9\]:

#strong[\1. Erro 401: Unauthorized]
...
#strong[\3. Resposta vazia ou None]
...
#strong[\4. Timeout na requisição]
...
#strong[\5. Erro de encoding (Unicode)]
...
#strong[Checklist de troubleshooting:] - \[ \] API key configurada e
válida - \[ \] Conexão com a internet funcionando - \[ \] Modelo
selecionado existe e está disponível - \[ \] Max\_tokens suficiente para
a resposta - \[ \] Temperature dentro da faixa válida (0.0-2.0) - \[ \]
Mensagens no formato correto (role + content)

#strong[Dicas para debugging eficiente:] 1. #strong[Imprima o payload
completo] antes de enviar 2. #strong[Use logging] em vez de print para
produção 3. #strong[Valide a resposta] antes de usar (verifique se não é
None) 4. #strong[Cache de erros] para evitar repetir requisições que
falharam 5. #strong[Teste com curl] primeiro para isolar o problema
(Python vs API)

== 2. Persistência e API: Memória do Assistente
<persistência-e-api-memória-do-assistente>
=== 2.1 Introdução
<introdução-1>
No capítulo anterior, construímos um chat funcional que mantém histórico
em memória. O problema? Quando você fecha o terminal, todo o histórico
se perde. Neste capítulo, vamos resolver dois problemas fundamentais:

+ #strong[Persistência:] Salvar conversas em um banco de dados para que
  nunca se percam
+ #strong[API REST:] Expor nosso chat como uma API que outros
  aplicativos podem usar

#strong[O que você vai adicionar ao projeto:] - FastAPI para criar
endpoints REST - PostgreSQL para persistir conversas - ORM (SQLAlchemy)
para interagir com o banco - Migrações de banco de dados - Testes
automatizados

#strong[Por que isso importa:] Todo sistema de IA em produção precisa de
persistência (para analytics, fallback, auditoria) e de uma API (para
integração com frontends, mobile, outros serviços). Estes são os
primeiros componentes de infraestrutura real do nosso assistente.

=== 2.2 Explica
<explica-1>
==== Arquitetura de APIs REST
<arquitetura-de-apis-rest>
REST (Representational State Transfer) é um padrão arquitetural para
APIs web \[1\]. Em vez de criar endpoints customizados para cada
operação, REST usa os verbos HTTP padrão:

#figure(
  align(center)[#table(
    columns: (38.71%, 32.26%, 29.03%),
    align: (auto,auto,auto,),
    table.header([Verbo HTTP], [Operação], [Exemplo],),
    table.hline(),
    [`GET`], [Ler dados], [`GET /conversas/123` --- buscar conversa],
    [`POST`], [Criar dados], [`POST /conversas` --- criar nova
    conversa],
    [`PUT`], [Atualizar dados], [`PUT /conversas/123` --- atualizar
    conversa],
    [`DELETE`], [Deletar dados], [`DELETE /conversas/123` --- remover
    conversa],
  )]
  , kind: table
  )

#strong[Por que REST e não GraphQL ou gRPC?] - REST é o padrão maisado e
bem documentado - Maioria dos clientes (web, mobile) já sabe consumir
REST - Ferramentas como FastAPI geram documentação automática (Swagger)
...
+ #strong[Conversas:] Uma sessão de chat (pode ter várias mensagens)
+ #strong[Mensagens:] Cada interação (usuário ou assistente)
+ #strong[Metadados:] Timestamps, tokens usados, modelo, latência

#strong[Schema do banco de dados:]
...
#strong[Por que UUID em vez de integer?] - UUIDs são únicos globalmente
(sem conflito entre bancos) - Não expõem a contagem de registros
(segurança) - Funcionam bem em sistemas distribuídos (futuro)

==== ORM com SQLAlchemy
<orm-com-sqlalchemy>
SQLAlchemy é o ORM (Object-Relational Mapping) mais usado no Python
\[3\]. Ele permite interagir com o banco de dados usando objetos Python
em vez de SQL raw:
...
#strong[Vantagens do ORM:] - Proteção contra SQL injection - Migrações
automáticas (Alembic) - Tipagem e autocompletar no editor - Facilidade
de testes (pode trocar o banco)

==== FastAPI: APIs Modernas no Python
<fastapi-apis-modernas-no-python>
FastAPI é um framework web moderno para Python que usa type hints para
gerar documentação automática \[4\]. Ele é ideal para APIs de IA porque:

- #strong[Alta performance:] Async/await nativo, tão rápido quanto
  Node.js
- #strong[Documentação automática:] Swagger UI gerada a partir dos type
  hints
- #strong[Validação automática:] Pydantic valida os dados de entrada
- #strong[Fácil de aprender:] Sintaxe simples e intuitiva
...
<migrações-com-alembic>
Alembic é a ferramenta de migração do SQLAlchemy \[5\]. Ele permite
versionar o schema do banco e aplicar mudanças incrementalmente:
...
#strong[Arquivo de migração (gerado automaticamente):]
...
<exercício-prático-setup-do-banco>
+ #strong[Inicie o PostgreSQL com Docker:]
...
#block[
#set enum(numbering: "1.", start: 4)
+ #strong[Teste os endpoints (Documentação automática):]
]
...
=== 2.6 Conclusão
<conclusão-1>
Neste capítulo, transformamos nosso chat simples em uma API profissional
com persistência. O projeto agora tem:

- #strong[FastAPI] com endpoints REST documentados
- #strong[PostgreSQL] persistindo conversas e mensagens
- #strong[SQLAlchemy ORM] com modelos tipados
- #strong[Alembic] para migrações versionadas
- #strong[Métricas] de tokens e latência em cada resposta
- #strong[Testes] automatizados

No próximo capítulo, vamos adicionar #strong[RAG (Retrieval-Augmented
Generation)] --- a capacidade de o assistente buscar informações em
documentos específicos antes de responder. Isso transformará nosso chat
genérico em um assistente especializado no conteúdo que você escolher.
...
\[2\] Microsoft. "Design a RAG Solution." Azure Architecture Center,
\2024. Disponível em:
https:/\/learn.microsoft.com/en-us/azure/architecture/ai-ml/

\[3\] SQLAlchemy. "SQLAlchemy 2.0 Documentation." SQLAlchemy Project,
\2024. Disponível em: https:/\/docs.sqlalchemy.org/

\[4\] FastAPI. "FastAPI --- Modern Python Web Framework." FastAPI
Documentation, 2024. Disponível em: https:/\/fastapi.tiangolo.com/

\[5\] Alembic. "Alembic --- Database Migration Tool." Alembic
Documentation, 2024. Disponível em: https:/\/alembic.sqlalchemy.org/

\[6\] PostgreSQL. "PostgreSQL 15 Documentation." PostgreSQL Global
Development Group, 2024. Disponível em:
https:/\/www.postgresql.org/docs/

\[7\] Docker. "Docker Compose Overview." Docker Documentation, 2024.
Disponível em: https:/\/docs.docker.com/compose/

\[8\] Pydantic. "Pydantic --- Data Validation." Pydantic Documentation,
\2024. Disponível em: https:/\/docs.pydantic.dev/

\[9\] Uvicorn. "Uvicorn --- ASGI Web Server." Uvicorn Documentation,
\2024. Disponível em: https:/\/www.uvicorn.org/

\[10\] Microsoft Azure Architecture Center. "Get Started with AI
Architecture Design." Microsoft Learn, 2024. Disponível em:
https:/\/learn.microsoft.com/en-us/azure/architecture/ai-ml/ai-get-started

\[11\] AWS. "Machine Learning Lens --- Well-Architected Framework."
Amazon Web Services, 2024. Disponível em:
https:/\/docs.aws.amazon.com/wellarchitected/latest/machine-learning-lens/

\[12\] Google Cloud. "ML System Design Patterns." Google Cloud
Architecture Center, 2023. Disponível em:
https:/\/cloud.google.com/architecture/ml-design-patterns

\[13\] Huyen, Chip. "Designing Machine Learning Systems." O'Reilly
Media, 2022. ISBN: 978-1098107963.

\[14\] DeepSeek. "API Documentation." DeepSeek API Docs, 2024.
Disponível em: https:/\/api-docs.deepseek.com/

\[15\] OpenAI. "API Reference." OpenAI Platform Documentation, 2024.
Disponível em: https:/\/platform.openai.com/docs/api-reference

\[16\] Pinecone. "What is RAG?" Pinecone Learning Center, 2024.
Disponível em:
https:/\/www.pinecone.io/learn/retrieval-augmented-generation/

\[17\] LangChain. "RAG from Scratch." LangChain Documentation, 2024.
Disponível em: https:/\/python.langchain.com/docs/tutorials/rag/

\[18\] OWASP. "Top 10 for Large Language Model Applications." OWASP
Foundation, 2024. Disponível em:
https:/\/owasp.org/www-project-top-10-for-large-language-model-applications/

\[19\] NIST. "Artificial Intelligence Risk Management Framework."
National Institute of Standards and Technology, 2024. Disponível em:
https:/\/www.nist.gov/artificial-intelligence/risk-management-framework

\[20\] Hugging Face. "PEFT Library." Hugging Face Documentation, 2024.
Disponível em: https:/\/huggingface.co/docs/peft

==== Versionamento de Dados e Schemas
<versionamento-de-dados-e-schemas>
Quando você muda o schema do banco, precisa de migrações seguras \[6\]:

#strong[Regras de ouro para migrações:]

+ #strong[Nunca delete dados] em migrações --- apenas adicione colunas
+ #strong[Mantenha compatibilidade] com versões anteriores
+ #strong[Teste migrações] antes de aplicar em produção
+ #strong[Tenha um rollback] para cada migração

#strong[Exemplo de migração segura:]
...
#strong[Versionamento de dados para IA:]
...
<rag-ensinando-o-assistente-a-buscar>
=== 3.1 Introdução
<introdução-2>
Nos capítulos anteriores, construímos um chat com persistência e API
REST. Mas nosso assistente ainda tem uma limitação fundamental: ele só
sabe o que foi treinado. Se você perguntar sobre um documento específico
da sua empresa, ele vai inventar uma resposta ou dizer que não sabe.

#strong[RAG (Retrieval-Augmented Generation)] resolve isso. É uma
técnica que permite ao assistente #strong[buscar informações relevantes]
em seus próprios documentos antes de gerar uma resposta \[1\]. Em vez de
confiar apenas no conhecimento do modelo, RAG combina:

+ #strong[Retrieval (Busca):] Encontrar trechos relevantes nos seus
  documentos
+ #strong[Augmented Generation (Geração Aprimorada):] Gerar uma resposta
  usando esses trechos como contexto

#strong[O que você vai construir:] - Pipeline de processamento de
documentos (chunking) - Sistema de embeddings vetoriais - Base de dados
vetorial com ChromaDB - Integração do RAG ao chat existente

#strong[Por que RAG é essencial:] - Reduz alucinações (o modelo cita
fontes reais) - Permite knowledge base atualizada sem retreinar - Mais
barato que fine-tuning para conhecimento específico - Transparência
(usuário pode ver as fontes)

=== 3.2 Explica
<explica-2>
==== O Problema das Alucinações
<o-problema-das-alucinações>
Quando um LLM não tem informação sobre um tópico, ele pode gerar texto
que parece correto mas é completamente inventado --- isso é chamado de
#strong[alucinação] \[2\]. Por exemplo:
...
<como-embeddings-funcionam>
Embeddings são representações vetoriais de texto \[3\]. Cada frase ou
parágrafo é convertido em um vetor de números (geralmente 384-1536
dimensões) que captura seu significado semântico.

#strong[Conceito chave:] Textos com significado similar ficam próximos
no espaço vetorial.
...
#strong[Distância cosine] mede a similaridade entre vetores: - 1.0 =
idênticos - 0.0 = completamente diferentes - Negativos = opostos

==== ChromaDB: Banco de Dados Vetorial
<chromadb-banco-de-dados-vetorial>
ChromaDB é um banco de dados vetorial open-source otimizado para IA
\[4\]. Ele permite:
...
#strong[Por que ChromaDB e não Pinecone/Weaviate?] - Local (sem custo de
nuvem) - Simples de configurar - Suficiente para projetos em estágio
inicial - Fácil de migrar para soluções maiores depois

==== Chunking: Dividindo Documentos
<chunking-dividindo-documentos>
Documentos grandes precisam ser divididos em pedaços menores (chunks)
antes de serem embeddidos \[5\]. Por quê?

+ #strong[Limite de tokens:] Modelos de embedding têm limite (geralmente
  512-8192 tokens)
+ #strong[Precisão da busca:] Chunks menores = trechos mais específicos
+ #strong[Qualidade da resposta:] Contexto relevante, não documentos
  inteiros

#strong[Estratégias de chunking:]

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([Estratégia], [Como funciona], [Quando usar],),
    table.hline(),
    [Fixo por tamanho], [Divide a cada N caracteres], [Documentos
    uniformes],
...
<ilustra-2>
==== Atualização do requirements.txt
<atualização-do-requirements.txt-1>
```txt
...
#strong[RAG Híbrido] combina busca vetorial com busca por palavras-chave
(BM25):
...
#strong[Vantagens do RAG Híbrido:] - BM25 pega palavras-chave exatas que
embeddings podem perder - Vetorial captura similaridade semântica -
Combinação \> qualquer abordagem isolada

#strong[Quando usar RAG Híbrido:] - Documentos com muitos termos
técnicos - Busca por códigos ou referências específicas - Quando a
precisão é mais importante que a recall

==== Chunking Avançado: Estratégias para Diferentes Tipos de Documento
<chunking-avançado-estratégias-para-diferentes-tipos-de-documento>
A estratégia de chunking impacta diretamente a qualidade do RAG \[7\].
Aqui está como escolher:

#strong[Para documentação técnica:]
...
#strong[Métricas de qualidade de chunking:]

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([Métrica], [O que mede], [Target],),
    table.hline(),
...
#strong[Dicas práticas:] 1. Comece com chunking por parágrafo (funciona
para 80% dos casos) 2. Ajuste max\_chars baseado no modelo de embedding
\3. Adicione overlap para preservar contexto entre chunks 4. Use
metadata para rastrear origem de cada chunk 5. Teste com 10-20 perguntas
reais para validar
...
=== 4.1 Introdução
<introdução-3>
Nos capítulos anteriores, construímos um assistente com chat,
persistência, API e RAG. O sistema funciona bem, mas ainda depende
completamente do modelo genérico. O que acontece quando você precisa de
um modelo que:
...
#strong[Fine-tuning] é o processo de #strong[treinar um modelo
existente] com dados específicos do seu domínio \[1\]. Em vez de criar
um modelo do zero (que custaria milhões de dólares), você adapta um
modelo já treinado para sua necessidade.

#strong[O que você vai aprender:] - Quando fazer fine-tuning vs.~usar
RAG - Preparação de dados de treino - Técnicas de fine-tuning eficientes
(LoRA, QLoRA) - Avaliação do modelo fine-tuned - Custos e trade-offs

#strong[Aviso importante:] Fine-tuning NÃO substitui RAG. Eles servem
para propósitos diferentes: - #strong[RAG:] Knowledge base atualizável,
respostas com fontes - #strong[Fine-tuning:] Comportamento, estilo,
formato, conhecimento fixo
...
+ #strong[Formato específico:] Você precisa que as respostas sigam um
  formato rígido (JSON, tabelas, código)
+ #strong[Jargão de domínio:] O modelo precisa entender termos técnicos
  específicos
+ #strong[Comportamento consistente:] Todas as respostas devem seguir um
  padrão
+ #strong[Redução de custo:] Respostas mais curtas = menos tokens =
  menos dinheiro
+ #strong[Latência:] Modelo fine-tuned pode ser menor e mais rápido

#strong[Quando NÃO fazer fine-tuning:]

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([Situação], [Usar RAG], [Usar Fine-Tuning],),
    table.hline(),
...
#strong[Dicas de preparação:]

+ #strong[Qualidade \> Quantidade:] 500 exemplos de alta qualidade \>
  5000 exemplos ruins
+ #strong[Diversidade:] Cubra todos os cenários que o modelo encontrará
+ #strong[Consistência:] Todas as respostas devem seguir o mesmo padrão
+ #strong[Limpeza:] Remova dados sensíveis, erros de digitação,
  respostas inconsistentes

==== LoRA: Fine-Tuning Eficiente
<lora-fine-tuning-eficiente>
LoRA (Low-Rank Adaptation) é uma técnica que treina apenas uma small
fraction dos parâmetros do modelo \[4\]. Em vez de ajustar bilhões de
parâmetros, LoRA treina matrizes de baixa dimensão:
...
#strong[Vantagens do LoRA:] - Treina em GPUs modestas (mesmo com 8GB de
VRAM) - É rápido (minutos vs horas) - Pode ser compartilhado (adaptador
leve) - Não degrada o modelo original

#strong[QLoRA] é ainda mais eficiente --- quantiza o modelo para 4 bits
durante o treino \[5\]:
...
+ #strong[Perplexity:] Mede quão "surpreso" o modelo fica com texto novo
  (menor = melhor)
+ #strong[BLEU/ROUGE:] Comparação com respostas de referência
+ #strong[Avaliação humana:] Pessoas avaliam a qualidade das respostas
+ #strong[Avaliação automática:] Outro LLM (GPT-4) julga as respostas
+ #strong[Métricas de domínio:] Acurácia em tarefas específicas
...
#block[
#set enum(numbering: "1.", start: 4)
+ #strong[Compare com o modelo base:]
]
...
=== 4.6 Conclusão
<conclusão-2>
Neste capítulo, você aprendeu a personalizar modelos de IA com
fine-tuning. O projeto agora tem:

- #strong[Pipeline de preparação de dados] para fine-tuning
- #strong[Fine-tuning com LoRA] que funciona em GPUs modestas
- #strong[Avaliação automatizada] da qualidade do modelo
- #strong[Calculadora de custos] para planejamento

No próximo capítulo, vamos implementar um #strong[sistema de evals] ---
avaliações automatizadas que garantem que o assistente responde
corretamente antes de ir para produção.

=== 4.7 Referências
<referências-2>
\[1\] OpenAI. "Fine-tuning Guide." OpenAI Platform Documentation, 2024.
Disponível em: https:/\/platform.openai.com/docs/guides/fine-tuning

\[2\] Huyen, Chip. "Designing Machine Learning Systems." O'Reilly Media,
\2022. ISBN: 978-1098107963.

\[3\] Hugging Face. "PEFT Library --- Parameter-Efficient Fine-Tuning."
Hugging Face Documentation, 2024. Disponível em:
https:/\/huggingface.co/docs/peft

\[4\] Hu, E.J. et al.~"LoRA: Low-Rank Adaptation of Large Language
Models." arXiv:2106.09685, 2021.

\[5\] Dettmers, T. et al.~"QLoRA: Efficient Finetuning of Quantized
Language Models." Advances in Neural Information Processing Systems,
vol.~35, 2022.

\[6\] DeepEval. "LLM Evaluation Framework." DeepEval Documentation,
\2024. Disponível em: https:/\/docs.confident-ai.com/

\[7\] Microsoft. "GenAI Operations with MLOps." Azure Architecture
Center, 2024. Disponível em:
https:/\/learn.microsoft.com/en-us/azure/architecture/ai-ml/
...
\[10\] DeepSeek. "API Documentation." DeepSeek API Docs, 2024.
Disponível em: https:/\/api-docs.deepseek.com/

\[11\] LangChain. "Fine-tuning Guide." LangChain Documentation, 2024.
Disponível em: https:/\/python.langchain.com/docs/

\[12\] Hugging Face. "Transformers Library." Hugging Face Documentation,
\2024. Disponível em: https:/\/huggingface.co/docs/transformers

\[13\] PyTorch. "PyTorch Documentation." PyTorch Project, 2024.
Disponível em: https:/\/pytorch.org/docs/

\[14\] NVIDIA. "NVIDIA Container Toolkit." NVIDIA Documentation, 2024.
Disponível em:
https:/\/docs.nvidia.com/datacenter/cloud-native/container-toolkit/

\[15\] Docker. "GPU Support with NVIDIA Container Toolkit." Docker
Documentation, 2024. Disponível em:
https:/\/docs.docker.com/gpu-support/

\[16\] OWASP. "Top 10 for Large Language Model Applications." OWASP
Foundation, 2024. Disponível em:
https:/\/owasp.org/www-project-top-10-for-large-language-model-applications/

\[17\] NIST. "Artificial Intelligence Risk Management Framework."
National Institute of Standards and Technology, 2024. Disponível em:
https:/\/www.nist.gov/artificial-intelligence/risk-management-framework

\[18\] Microsoft Azure Architecture Center. "Get Started with AI
Architecture Design." Microsoft Learn, 2024. Disponível em:
https:/\/learn.microsoft.com/en-us/azure/architecture/ai-ml/ai-get-started

\[19\] Pinecone. "What is RAG?" Pinecone Learning Center, 2024.
Disponível em:
https:/\/www.pinecone.io/learn/retrieval-augmented-generation/

\[20\] Prometheus. "Monitoring Best Practices." Prometheus
Documentation, 2024. Disponível em: https:/\/prometheus.io/docs/

==== Comparação de Técnicas de Fine-Tuning
<comparação-de-técnicas-de-fine-tuning>
Entender quando usar cada técnica é crucial para otimizar custos e
qualidade \[6\]:

#figure(
  align(center)[#table(
    columns: (13.64%, 33.33%, 25.76%, 10.61%, 16.67%),
    align: (auto,auto,auto,auto,auto,),
    table.header([Técnica], [Parâmetros Treináveis], [VRAM
      Necessária], [Tempo], [Qualidade],),
    table.hline(),
...
#strong[Full Fine-Tuning:] - Treina TODOS os parâmetros do modelo -
Melhor qualidade, mas caro e lento - Requer GPU com muita VRAM (A100,
H100) - Usar quando: dataset grande, qualidade é prioridade

#strong[LoRA (Low-Rank Adaptation):] - Treina apenas matrizes de baixa
dimensão - 99% menos parâmetros que full fine-tuning - Funciona em GPUs
modestas (T4, V100) - Usar quando: orçamento limitado, qualidade
aceitável

#strong[QLoRA:] - LoRA + quantização 4 bits - Ainda mais eficiente que
LoRA - Funciona em GPUs com 4GB de VRAM - Usar quando: hardware muito
limitado

#strong[Prompt Tuning:] - Aprende um "prompt" contínuo (não texto) -
Extremamente leve - Qualidade inferior a LoRA - Usar quando: muitas
tarefas, poucos dados

#strong[In-Context Learning:] - Não treina nada, só usa exemplos no
prompt - Zero custo de treino - Qualidade depende dos exemplos - Usar
quando: prototipagem rápida

#strong[Fluxo de decisão:]
...
#strong[Erros comuns de iniciantes:] 1. #strong[Fine-tuning com dados
ruins:] Qualidade \> Quantidade 2. #strong[Esquecer de avaliar:] Sem
evals, você não sabe se melhorou 3. #strong[Overfitting:] Modelo decora
exemplos em vez de aprender padrões 4. #strong[Ignorar custos:]
Fine-tuning custa GPU, não é grátis 5. #strong[Não testar em produção:]
Lab ≠ Produção

== 5. Evals e Testing: Qualidade Garantida
<evals-e-testing-qualidade-garantida>
=== 5.1 Introdução
<introdução-4>
Nos capítulos anteriores, construímos um assistente de IA completo: chat
com persistência, API REST, RAG para conhecimento e fine-tuning para
personalização. Mas como ter certeza de que ele funciona
#strong[corretamente] antes de colocar em produção?

#strong[Evals (avaliações)] são o processo de testar sistematicamente a
qualidade de um sistema de IA \[1\]. Diferente de testes tradicionais
onde "funcionar" significa "não dar erro", em IA "funcionar" significa
"gerar respostas úteis, precisas e seguras".

#strong[O que você vai construir:] - Framework de avaliação automatizada
\- Métricas relevantes para sistemas de IA - Benchmarking contínuo -
CI/CD para qualidade de IA

#strong[Por que evals importam:] - Um chatbot que responde informações
erradas pode causar danos reais - Regressões de qualidade são difícis de
detectar sem evals - Métricas objetivas substituem "feeling" na
avaliação - Evals automáticos permitem iteração rápida

=== 5.2 Explica
<explica-4>
==== Por que Testes Tradicionais Não Funcionam para IA
<por-que-testes-tradicionais-não-funcionam-para-ia>
Em software tradicional, um teste verifica se uma função retorna o valor
esperado:
...
Em IA, a "resposta correta" pode variar dependendo de: - Contexto da
conversa - Formato esperado - Nível de detalhe - Fonte citada - Tom de
voz

Por isso, evals de IA usam #strong[métricas probabilísticas] em vez de
asserts exatos \[2\].
...
#figure(
  align(center)[#table(
    ...
    align: (auto,auto,auto,),
    table.header([Métrica], [O que mede], [Como calcular],),
    table.hline(),
    [#strong[Faithfulness]], [A resposta é fiel ao contexto?], [LLM
    julga se cada afirmação é suportada],
    [#strong[Relevancy]], [A resposta é relevante para a
    pergunta?], [LLM julga se a resposta endereça a pergunta],
    [#strong[Answer Correctness]], [A resposta está factualmente
    correta?], [Comparação com ground truth],
    [#strong[Context Precision]], [Os documentos recuperados são
    relevantes?], [Precisão dos top-K resultados],
    [#strong[Context Recall]], [Todos os documentos relevantes foram
    recuperados?], [Recall da recuperação],
  )]
  , kind: table
  )
...
+ #strong[Dataset de teste:] Perguntas com respostas esperadas
+ #strong[Métricas:] Definições objetivas de qualidade
+ #strong[Execução automática:] Roda evals sem intervenção humana
+ #strong[Relatórios:] Visualização clara dos resultados
+ #strong[Alertas:] Notificação quando a qualidade cai

==== Benchmarking Contínuo
<benchmarking-contínuo>
Benchmarking não é algo que você faz uma vez --- é um processo contínuo
\[4\]:
...
Isso é especialmente importante em sistemas de IA porque: - Modelos de
API podem mudar sem aviso - Dados de treino podem ficar desatualizados -
Novos cenários podem surgir - Performance pode degradar com o tempo
...
#strong[Componentes do CI/CD para IA:]

+ #strong[Testes unitários:] Código funciona?
+ #strong[Testes de integração:] Componentes se comunicam?
+ #strong[Evals:] Respostas são de qualidade?
+ #strong[Segurança:] Não há vulnerabilidades?
+ #strong[Performance:] Latência aceitável?
+ #strong[Custo:] Orçamento não estourado?

#strong[Métricas de qualidade no CI:]

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([Métrica], [Threshold], [Ação se falhar],),
    table.hline(),
...
#strong[Rollback automático:] Se os evals caírem após um deploy, o CI
deve: 1. Detectar a regressão 2. Notificar o time 3. automaticamente
reverter para a versão anterior 4. Criar issue para investigação

==== Criando um Dataset de Teste Robusto
<criando-um-dataset-de-teste-robusto>
Um bom dataset de teste é a base de evals confiáveis \[8\]. Aqui está
como criá-lo:

#strong[Estrutura do dataset:]
...
#strong[Dicas para criar bons datasets:]

+ #strong[Diversidade:] Cubra diferentes tipos de perguntas
...
#strong[Automação de geração de datasets:]
...
<métricas-avançadas-e-dashboards>
Além das métricas básicas, sistemas de IA em produção precisam de
métricas avançadas \[9\]:

#strong[Métricas de negócio:] - #strong[Satisfação do usuário:] Média de
ratings (1-5) após respostas - #strong[Taxa de resolução:] % de
perguntas respondidas sem intervenção humana - #strong[Tempo para
primeira resposta:] Latência percebida pelo usuário - #strong[Retenção:]
Usuários que voltam a usar o assistente

#strong[Métricas de qualidade:] - #strong[Hallucination rate:] % de
respostas com informações inventadas - #strong[Citation accuracy:] % de
respostas com fontes corretas - #strong[Format compliance:] % de
respostas no formato esperado - #strong[Safety score:] % de respostas
que passam no filtro de segurança

#strong[Dashboard de Evals:]
...
#strong[Integração com Slack/Teams:]
...
<segurança-e-controle-auth-e-rate-limiting>
=== 6.1 Introdução
<introdução-5>
Nos capítulos anteriores, construímos um assistente de IA completo com
persistência, RAG, fine-tuning e sistema de evals. Mas ainda falta algo
crítico para produção: #strong[segurança e controle de acesso].

Sem autenticação, qualquer pessoa pode usar e abusar do seu assistente.
Sem rate limiting, um único usuário pode sobrecarregar o sistema e gerar
custos inesperados. Sem proteção contra prompt injection, atacantes
podem manipular o comportamento do assistente.

#strong[O que você vai adicionar:] - Autenticação JWT (JSON Web Tokens)
\- Autorização baseada em papéis (RBAC) - Rate limiting para controle de
uso - Proteção contra prompt injection - Auditoria de segurança

#strong[Por que isso é urgente:] - OWASP listou 10 vulnerabilidades
específicas para LLMs \[1\] - Prompt injection pode expor dados
sensíveis - Custos de API podem explodir sem controle - Compliance
(LGPD, GDPR) exige auditoria

=== 6.2 Explica
<explica-5>
==== Autenticação em APIs de IA
<autenticação-em-apis-de-ia>
Autenticação verifica #strong[quem] está fazendo a requisição \[2\]. Em
APIs de IA, os métodos comuns são:

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([Método], [Como funciona], [Quando usar],),
    table.hline(),
    [API Key], [Chave estática no header], [Serviços internos, MVP],
    [JWT], [Token assinado com expiração], [APIs públicas,
    multi-tenant],
    [OAuth2], [Token de terceiros (Google, GitHub)], [Login social,
    SSO],
...
#strong[Para nosso assistente, usaremos JWT] porque: - Stateless (não
precisa consultar banco a cada requisição) - Suporta multi-tenant (cada
usuário tem seus dados) - Padrão da indústria - Fácil de implementar com
FastAPI
...
<rate-limiting-controle-de-uso>
Rate limiting restringe quantas requisições um usuário pode fazer em um
período \[3\]. É essencial para:

+ #strong[Prevenir abuso:] Bloquear bots e ataques de força bruta
+ #strong[Controlar custos:] Limitar gasto por usuário
+ #strong[Garantir disponibilidade:] Evitar sobrecarga do sistema
+ #strong[Fair use:] Distribuir recursos justamente

#strong[Estratégias de rate limiting:]

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([Estratégia], [Como funciona], [Exemplo],),
    table.hline(),
    ...
    [Leaky Bucket], [Fila com processamento fixo], [10 req/min, fila de
    20],
  )]
  , kind: table
  )
...
<proteção-contra-prompt-injection>
Prompt injection é quando um atacante insere instruções maliciosas na
entrada do usuário para manipular o comportamento do LLM \[4\].
Exemplos:
...
#strong[Técnicas de proteção:]

+ #strong[Validação de entrada:] Sanitizar texto antes de enviar ao LLM
+ #strong[Separação de contexto:] Usar delimitadores claros entre
  instruções e input
+ #strong[Treinamento:] Fine-tuning com exemplos de injection
+ #strong[Monitoramento:] Detectar padrões suspeitos em tempo real
...
==== Proteção contra Prompt Injection
<proteção-contra-prompt-injection-1>
```python
...
#strong[Checklist de compliance:] - \[ \] Política de privacidade
atualizada - \[ \] Consentimento coletado antes do primeiro uso - \[ \]
Dados criptografados em trânsito e repouso - \[ \] Direito de acesso
implementado (exportar dados) - \[ \] Direito de esquecimento
implementado (deletar dados) - \[ \] Registro de operações de tratamento
\- \[ \] DPO (Data Protection Officer) designado - \[ \] Relatório de
impacto à proteção de dados (RIPD)

== 7. Deploy e Monitoramento: Em Produção
<deploy-e-monitoramento-em-produção>
=== 7.1 Introdução
<introdução-6>
Nos capítulos anteriores, construímos um assistente de IA completo:
chat, persistência, API, RAG, fine-tuning, evals, autenticação e rate
limiting. Tudo funciona no desenvolvimento. Mas a verdadeira prova é:
#strong[funciona em produção?]

Deploy e monitoramento são o que separa um protótipo de um sistema
profissional \[1\]. Neste capítulo, você vai:
...
#strong[Por que isso é essencial:] - "Funciona na minha máquina" não é
suficiente - Sem monitoramento, você só descobre problemas quando os
usuários reclamam - Logs estruturados permitem debug eficiente - Alertas
automáticos evitam downtime

=== 7.2 Explica
<explica-6>
==== Containerização de IA
<containerização-de-ia>
Docker permite empacotar sua aplicação com todas as dependências em um
container portável \[2\]. Para sistemas de IA, isso é especialmente
importante porque:

+ #strong[Dependências complexas:] PyTorch, CUDA, bibliotecas de ML
+ #strong[Reprodutibilidade:] Mesmo ambiente em qualquer lugar
+ #strong[Isolamento:] Cada serviço em seu container
+ #strong[Escalabilidade:] Fácil de escalar horizontalmente

#strong[Arquitetura com Docker:]
...
<monitoramento-com-prometheus>
Prometheus é o padrão de monitoramento para sistemas distribuídos \[3\].
Ele coleta métricas em tempo real e permite consultas poderosas:

#strong[Métricas essenciais para IA:]

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([Métrica], [O que mede], [Por que importa],),
    table.hline(),
    [`requests_total`], [Total de requisições], [Volume de uso],
    [`request_duration_seconds`], [Latência das
...
#strong[Formato de métricas Prometheus:]
...
<grafana-visualização-de-métricas>
Grafana cria dashboards interativos a partir de métricas do Prometheus
\[4\]. Para um sistema de IA, o dashboard deve mostrar:

+ #strong[Visão geral:] Request rate, error rate, latência (os "golden
  signals")
+ #strong[Custo:] Tokens consumidos por modelo, custo estimado
+ #strong[Qualidade:] Taxa de sucesso dos evals, alucinações detectadas
+ #strong[Infraestrutura:] CPU, memória, GPU (se aplicável)

==== Logs Estruturados
<logs-estruturados>
Logs estruturados são logs em formato JSON que facilitam busca e análise
\[5\]:
...
#strong[Por que logs estruturados?] - Busca por campos específicos
(findAll where user\_id = X) - Análise de padrões (top 10 users by token
usage) - Integração com ferramentas (ELK Stack, Datadog) - Debug
eficiente (filtrar por erro específico)
...
<exercício-prático-deploy-completo>
+ #strong[Build das imagens Docker:]
...
#block[
#set enum(numbering: "1.", start: 2)
+ #strong[Subir todos os serviços:]
]
...
=== 7.6 Conclusão
<conclusão-3>
Neste capítulo, seu assistente de IA está pronto para produção. O
sistema agora tem:

- #strong[Docker] com containers otimizados
- #strong[Docker Compose] com todos os serviços
- #strong[Prometheus] coletando métricas
- #strong[Grafana] visualizando dashboards
- #strong[Alertas] automáticos para problemas
- #strong[Logs estruturados] para debug
- #strong[Health checks] para orquestradores

No próximo capítulo, vamos evoluir para uma #strong[arquitetura
avançada] com cache, fallback de modelos e multi-tenancy.

=== 7.7 Referências
<referências-3>
\[1\] Docker. "Containerization Best Practices." Docker Documentation,
\2024. Disponível em: https:/\/docs.docker.com/

\[2\] Kubernetes. "Production-Grade Container Orchestration." Kubernetes
Documentation, 2024. Disponível em: https:/\/kubernetes.io/docs/

\[3\] Prometheus. "Monitoring Best Practices." Prometheus Documentation,
\2024. Disponível em: https:/\/prometheus.io/docs/

\[4\] Grafana. "Dashboard Design." Grafana Documentation, 2024.
Disponível em: https:/\/grafana.com/docs/

\[5\] Structured Logging. "Why Structured Logging Matters." O'Reilly,
\2023.

\[6\] Microsoft. "GenAI Operations with MLOps." Azure Architecture
Center, 2024. Disponível em:
https:/\/learn.microsoft.com/en-us/azure/architecture/ai-ml/

\[7\] AWS. "Machine Learning Lens --- Well-Architected Framework."
Amazon Web Services, 2024. Disponível em:
https:/\/docs.aws.amazon.com/wellarchitected/latest/machine-learning-lens/

\[8\] Google Cloud. "ML System Design Patterns." Google Cloud
Architecture Center, 2023. Disponível em:
https:/\/cloud.google.com/architecture/ml-design-patterns

\[9\] Huyen, Chip. "Designing Machine Learning Systems." O'Reilly Media,
\2022. ISBN: 978-1098107963.

\[10\] DeepSeek. "API Documentation." DeepSeek API Docs, 2024.
Disponível em: https:/\/api-docs.deepseek.com/

\[11\] FastAPI. "Production Deployment." FastAPI Documentation, 2024.
Disponível em: https:/\/fastapi.tiangolo.com/deployment/

\[12\] Uvicorn. "Uvicorn --- ASGI Web Server." Uvicorn Documentation,
\2024. Disponível em: https:/\/www.uvicorn.org/

\[13\] PostgreSQL. "PostgreSQL 15 Documentation." PostgreSQL Global
Development Group, 2024. Disponível em:
https:/\/www.postgresql.org/docs/

\[14\] Redis. "Redis Documentation." Redis Ltd., 2024. Disponível em:
https:/\/redis.io/docs/

\[15\] OWASP. "Top 10 for Large Language Model Applications." OWASP
Foundation, 2024. Disponível em:
https:/\/owasp.org/www-project-top-10-for-large-language-model-applications/
...
\[17\] Microsoft Azure Architecture Center. "Get Started with AI
Architecture Design." Microsoft Learn, 2024. Disponível em:
https:/\/learn.microsoft.com/en-us/azure/architecture/ai-ml/ai-get-started

\[18\] Pinecone. "What is RAG?" Pinecone Learning Center, 2024.
Disponível em:
https:/\/www.pinecone.io/learn/retrieval-augmented-generation/

\[19\] LangChain. "RAG from Scratch." LangChain Documentation, 2024.
Disponível em: https:/\/python.langchain.com/docs/tutorials/rag/
...
<estratégias-de-escalabilidade>
Quando seu assistente cresce de 10 para 10.000 usuários, você precisa de
escalabilidade \[8\]:

#strong[Escalabilidade Horizontal (Scale Out):]
...
#strong[Escalabilidade Vertical (Scale Up):] Aumentar recursos do
servidor (mais RAM, CPU, GPU). Mais simples mas tem limite.

#strong[Estratégias específicas para IA:]

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([Estratégia], [Quando usar], [Custo],),
    table.hline(),
...
#strong[Auto-scaling com Kubernetes:]
...
#strong[Monitoramento de capacidade:] - Alertar quando CPU \> 70% por 5
minutos - Escalar automaticamente quando fila \> 100 requisições -
Desescalar quando carga cair por 10 minutos

==== Debugging em Produção
<debugging-em-produção>
Quando algo dá errado em produção, você precisa de ferramentas rápidas
para diagnosticar \[10\]:

#strong[Estratégia de debugging:]

+ #strong[Logs estruturados:] Busca por campos específicos
...
#block[
#set enum(numbering: "1.", start: 2)
+ #strong[Distributed tracing:] Acompanhar uma requisição através de
  múltiplos serviços
]
...
#block[
#set enum(numbering: "1.", start: 3)
+ #strong[Métricas customizadas para debugging:]
]
...
#strong[Ferramentas recomendadas:] - #strong[Jaeger/Zipkin:] Distributed
tracing - #strong[Sentry:] Error tracking - #strong[Datadog/Grafana
Cloud:] APM completo - #strong[PagerDuty:] Alertas e escalonamento

== 8. Arquitetura Avançada: Escalando o Sistema
<arquitetura-avançada-escalando-o-sistema>
=== 8.1 Introdução
<introdução-7>
Neste último capítulo, vamos transformar nosso assistente de IA
funcional em um #strong[sistema de produção escalável]. Nos capítulos
anteriores, construímos cada componente individualmente. Agora, vamos
conectá-los com uma arquitetura que suporta:

- #strong[Milhares de usuários] simultâneos
- #strong[Múltiplos modelos] com fallback automático
- #strong[Cache inteligente] para reduzir custos
- #strong[Multi-tenancy] para isolamento de clientes
- #strong[Orquestração] de pipelines complexos

#strong[O que você vai construir:] - Orquestrador de modelos com
fallback - Cache de respostas semântico - Sistema multi-tenant completo
\- Pipeline de processamento avançado - Otimização de custos

#strong[Por que esta arquitetura importa:] - Sistemas reais não usam um
único modelo - Cada centavo conta quando você processa milhões de
requisições - Isolamento de dados é obrigatório em enterprise - Fallback
garante disponibilidade mesmo com falhas

=== 8.2 Explica
<explica-7>
==== Padrões de Arquitetura Avançada
<padrões-de-arquitetura-avançada>
A Microsoft identificou padrões comuns em sistemas de IA em produção
\[1\]:

#strong[\1. Gateway Pattern:] Um gateway roteia requisições para
múltiplos backends de modelo. Benefícios: - Balanceamento de carga -
Cache centralizado - Rate limiting unificado - Observabilidade
consolidada

#strong[\2. Circuit Breaker Pattern:] Quando um modelo falha, o sistema
automaticamente muda para outro. Benefícios: - Disponibilidade mesmo com
falhas - Degradção graciosa - Prevenção de cascata de falhas

#strong[\3. Strangler Fig Pattern:] Migrar gradualmente de um modelo
para outro. Benefícios: - Zero downtime - Rollback fácil - Comparação
A/B em produção

==== Cache Semântico
<cache-semântico>
Cache tradicional usa chaves exatas. Cache semântico usa
#strong[similaridade] \[2\]:
...
#strong[Como funciona:] 1. Cada pergunta é convertida em embedding 2.
Antes de chamar a IA, busca no cache por similaridade 3. Se encontrar
algo similar o suficiente (cosine \> 0.95), retorna a resposta cacheada
\4. Se não, chama a IA e salva no cache

#strong[Economia típica:] 30-50% de redução em chamadas à API

==== Multi-Tenancy
<multi-tenancy>
Multi-tenancy permite que múltiplos clientes usem o mesmo sistema com
dados isolados \[3\]:
...
#strong[Níveis de multi-tenancy:]

#figure(
  align(center)[#table(
    columns: 4,
    align: (auto,auto,auto,auto,),
    table.header([Nível], [Isolamento], [Custo], [Complexidade],),
    table.hline(),
...
Para a maioria dos casos, #strong[Shared DB com Row-Level Security] é
suficiente.

==== Orquestração de Modelos
<orquestração-de-modelos>
Em produção, você não usa um único modelo. Usa múltiplos para diferentes
tarefas \[4\]:
...
O orquestrador decide qual modelo usar baseado em: - Tipo de tarefa -
Complexidade estimada - Custo disponível - Disponibilidade do modelo
...
#strong[Capítulo 1:] Chat básico com API ✓ #strong[Capítulo 2:]
Persistência + API REST ✓ #strong[Capítulo 3:] RAG com ChromaDB ✓
#strong[Capítulo 4:] Fine-tuning com LoRA ✓ #strong[Capítulo 5:] Sistema
de evals ✓ #strong[Capítulo 6:] Auth + Rate Limiting ✓ #strong[Capítulo
7:] Docker + Monitoramento ✓ #strong[Capítulo 8:] Arquitetura avançada ✓

#strong[Stack completa:] - Python 3.11+ / FastAPI - PostgreSQL + Redis -
ChromaDB (vetor) - OpenAI/DeepSeek API - Docker + docker-compose -
Prometheus + Grafana - JWT + OAuth2 - LoRA/QLoRA (fine-tuning)
...
=== 8.6 Conclusão
<conclusão-4>
Parabéns! Você construiu um sistema de IA completo e profissional. Cada
capítulo adicionou uma camada real ao projeto, e agora você tem:

+ #strong[Conhecimento técnico] para construir sistemas de IA
+ #strong[Arquitetura escalável] que suporta crescimento
+ #strong[Boas práticas] de segurança, monitoramento e custos
+ #strong[Projeto portfolio] que demonstra suas habilidades

#strong[Próximos passos:] - Deploy em produção real (AWS, GCP, Azure) -
Integração com frontends (React, Next.js) - Pipelines de CI/CD
automatizados - Contribuição para projetos open-source de IA

Lembre-se: todo grande sistema começa com uma chamada simples que
funciona. Você deu o primeiro passo no capítulo 1 e agora tem um sistema
completo. Continue construindo!

=== 8.7 Referências
<referências-4>
\[1\] Microsoft. "AI Agent Orchestration Patterns." Azure Architecture
Center, 2024. Disponível em:
https:/\/learn.microsoft.com/en-us/azure/architecture/ai-ml/

\[2\] Google Cloud. "Caching Strategies for ML Systems." Google Cloud
Architecture Center, 2023. Disponível em:
https:/\/cloud.google.com/architecture/ml-design-patterns

\[3\] AWS. "Multi-Tenant Architecture on AWS." Amazon Web Services,
\2024. Disponível em:
https:/\/docs.aws.amazon.com/wellarchitected/latest/machine-learning-lens/

\[4\] LangChain. "Model Router Pattern." LangChain Documentation, 2024.
Disponível em: https:/\/python.langchain.com/docs/

\[5\] DeepSeek. "API Documentation." DeepSeek API Docs, 2024. Disponível
em: https:/\/api-docs.deepseek.com/

\[6\] Huyen, Chip. "Designing Machine Learning Systems." O'Reilly Media,
\2022. ISBN: 978-1098107963.

\[7\] Prometheus. "Monitoring Best Practices." Prometheus Documentation,
\2024. Disponível em: https:/\/prometheus.io/docs/

\[8\] Grafana. "Dashboard Design." Grafana Documentation, 2024.
Disponível em: https:/\/grafana.com/docs/

\[9\] Docker. "Containerization Best Practices." Docker Documentation,
\2024. Disponível em: https:/\/docs.docker.com/

\[10\] FastAPI. "Production Deployment." FastAPI Documentation, 2024.
Disponível em: https:/\/fastapi.tiangolo.com/deployment/

\[11\] OWASP. "Top 10 for Large Language Model Applications." OWASP
Foundation, 2024. Disponível em:
https:/\/owasp.org/www-project-top-10-for-large-language-model-applications/
...
\[13\] Microsoft Azure Architecture Center. "Get Started with AI
Architecture Design." Microsoft Learn, 2024. Disponível em:
https:/\/learn.microsoft.com/en-us/azure/architecture/ai-ml/ai-get-started

\[14\] Pinecone. "What is RAG?" Pinecone Learning Center, 2024.
Disponível em:
https:/\/www.pinecone.io/learn/retrieval-augmented-generation/

\[15\] OpenAI. "Fine-tuning Guide." OpenAI Platform Documentation, 2024.
Disponível em: https:/\/platform.openai.com/docs/guides/fine-tuning

\[16\] Hugging Face. "PEFT Library." Hugging Face Documentation, 2024.
Disponível em: https:/\/huggingface.co/docs/peft

\[17\] DeepEval. "LLM Evaluation Framework." DeepEval Documentation,
\2024. Disponível em: https:/\/docs.confident-ai.com/

\[18\] RAGAS. "RAG Evaluation Framework." RAGAS Documentation, 2024.
Disponível em: https:/\/docs.ragas.io/

\[19\] Kubernetes. "Production-Grade Container Orchestration."
Kubernetes Documentation, 2024. Disponível em:
https:/\/kubernetes.io/docs/

\[20\] Redis. "Redis Documentation." Redis Ltd., 2024. Disponível em:
https:/\/redis.io/docs/
